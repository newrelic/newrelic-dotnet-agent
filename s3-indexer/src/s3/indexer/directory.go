...
func (d *Directory) Enumerate(svc *s3.S3, bucket, prefix string) error {
	...
	if err := svc.ListObjectsV2Pages(params, func(resp *s3.ListObjectsV2Output, lastPage bool) bool {
		for _, obj := range resp.Contents {
			...
			// Another special case: S3 helpfully returns the prefix you just
			// provided as a file. Since we don't want a recursive loop of entries,
			// we'll also ignore that.
			if file.Name == strings.TrimRight(prefix, "/") {
				continue
			}
