...
namespace CompositeTests
{
	...
	public class DatastoreSegmentQueryParametersTests
	{
		...
		{
			var reallyLongString = new string('a', AgentWrapperApi.QueryParameterMaxStringLength + 1);
			...
			var truncatedName = new string('a', AgentWrapperApi.QueryParameterMaxStringLength);
			CollectionAssert.AreEquivalent(segmentData.QueryParameters, new Dictionary<string, IConvertible>
			{
		...
		public void DatastoreSegment_ShouldHandleBadResultsFromObjectsConvertedToStrings()
		{
			...
			NrAssert.Multiple(
				...
				() => Assert.AreEqual(new string('l', AgentWrapperApi.QueryParameterMaxStringLength), segmentData.QueryParameters["longConvertible"])
			);
		}
		...
		private TypedSegment<DatastoreSegmentData> CreateWebTransactionWithDatastoreSegment(IDictionary<string, IConvertible> queryParameters)
		{
			...
			return (TypedSegment<DatastoreSegmentData>)_agentWrapperApi.StartDatastoreRequestSegmentOrThrow("INSERT", DatastoreVendor.MSSQL, "MyAwesomeTable", null, null, "HostName", "1433", "MyDatabase", queryParameters);
		}
		...
		private class LongStringConvertible : IConvertible
		{
			...
			public string ToString(IFormatProvider provider)
			{
				return new string('l', AgentWrapperApi.QueryParameterMaxStringLength + 1);
			}
