...
namespace NewRelic.Agent.Core.Spans.Tests
{
	...
	{
		private readonly IAttributeDefinitionService _attribDefSvc = new AttributeDefinitionService((f)=>new AttributeDefinitions(f));
		...
		public void SpanEventWireModelTests_Serialization()
		{
			...
			_attribDefs.Type.TrySetValue(spanEventWireModel, TypeAttributeValue.Span);
		...
		private ISpanEventWireModel CreateSpanEvent(float priority, TimeSpan duration, string traceId, string httpMethod)
		{
			...
			_attribDefs.Type.TrySetValue(wireModel, TypeAttributeValue.Span);
