...
namespace NewRelic.Providers.Wrapper.Elasticsearch
{
    ...
    public class ElasticTransportRequestWrapper : IWrapper
    {
        ...
        {
            //Elastic.Transport.HttpMethod,System.String,Elastic.Transport.PostData,Elastic.Transport.RequestParameters
            ...
            var model = path.Split('/')[1]; // "model"=table name for SQL.  For elastic it's index name.  v8 looks like '/people/whatever'
            ...
            if (typeOfRequestParams.Name == "IndexRequestParameters")
            {
                operation = "Index";
            }
            else if (typeOfRequestParams.Name == "SearchRequestParameters")
            {
                operation = "Search";
            }
            // etc, etc
