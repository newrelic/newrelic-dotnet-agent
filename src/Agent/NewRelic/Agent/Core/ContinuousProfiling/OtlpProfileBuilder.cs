...
public static class OtlpProfileBuilder
{
    ...
    // profiles.proto defines cpu/off_cpu/allocated_objects/allocated_space as sample types, and
    // Profile.sample_type is singular. This builder previously split resolved samples into two
    // Profile messages (cpu = on-CPU-only, off_cpu = parked-only) sharing this request's
    // ProfilesDictionary. cpu and off_cpu were merged back into a single "cpu" Profile covering
    // every resolved sample regardless of on/off-CPU state -- the on/off-CPU classification signal
    // is no longer captured anywhere (native, wire format, or here).
    //
    // period_type for the merged profile is ("cpu","nanoseconds") -- OTel's own pairing for a plain
    // cpu-sample_type profile (profiles.proto: "sample_type=(cpu, milliseconds),
    // period_type=(cpu, milliseconds)").
    //
    // Accuracy limit at high thread counts: the native profiler samples at most a fixed number of
    // threads (~capacity) per tick and round-robins the rest over successive ticks (see
    // PlanCaptureWindow in ContinuousProfiler.h). So in a process with more live threads than that
    // capacity, each individual thread is effectively sampled at 1/ceil(threadCount/capacity) of the
    // configured cadence, yet every sample here is still attributed the full `period`. Per-thread
    // and aggregate totals are therefore proportionally under-counted for apps well over the
    // capacity (e.g. large IIS/ASP.NET hosts). This is an inherent fixed-capacity tradeoff; the
    // native side logs an overflow counter each tick so the under-sampling is observable rather than
    // silent. Flame-graph SHAPE (relative frame proportions) stays representative; absolute time
    // totals are the lossy dimension.
    ...
    // How many nanoseconds of a thread's time one emitted sample represents. Not always the configured
    // interval: a capture overrun lengthens the real tick period (native measures and ships it as
    // ActualSamplePeriodMs, preferred when present), and round-robin under-sampling (more live threads
    // than capture capacity, see PlanCaptureWindow in ContinuousProfiler.h) means one emitted sample
    // stands in for ceil(total/emitted) periods of that thread's time -- without scaling, a busy
    // 400-thread host's per-thread totals would read ~4x low. deferred == 0 yields factor 1 (unchanged
    // behavior). Public + static so it is directly unit-testable.
    public static long ComputeSampleValueNanos(long configuredPeriodNanos, long actualSamplePeriodMs, int emittedThreadCount, int deferredThreadCount)
    {
        var baseNanos = actualSamplePeriodMs > 0 ? actualSamplePeriodMs * 1_000_000L : configuredPeriodNanos;

        // Round-robin correction: factor = ceil(total / emitted). Guarded on emitted > 0 (no divide-by-zero)
        // and deferred > 0 (nothing deferred -> no under-sampling -> factor 1, leave baseNanos untouched).
        if (emittedThreadCount > 0 && deferredThreadCount > 0)
        {
            var total = (long)emittedThreadCount + deferredThreadCount;
            var factor = (total + emittedThreadCount - 1) / emittedThreadCount; // integer ceil(total/emitted)
            return baseNanos * factor;
        }

        return baseNanos;
    }

    // perSampleValueNanos: the time each emitted sample represents (see ComputeSampleValueNanos). 0 means "use
    // the configured periodNanos" -- the historical behavior every caller/test that doesn't pass it still gets.
    // The informational profile.Period stays the configured interval regardless; only the per-sample VALUE is
    // adjusted, so overrun/round-robin corrections don't distort the reported cadence.
    public static ExportProfilesServiceRequest Build(IReadOnlyList<ManagedThreadSample> samples, long startUnixNano, long durationNano, string serviceName, string entityGuid = null, string host = null, long periodNanos = 0, bool includeAgentCode = true, long perSampleValueNanos = 0)
    {
        ...
        var threadIdRegistry = ManagedThreadIdRegistry.Instance;
        foreach (var sample in samples)
        {
            ...
            // A sample with empty Frames previously interned to stack_index 0, which is also the reserved
            // "no stack" sentinel (see the stackTable[string.Empty] = 0 wiring above) -- an empty-stack sample
            // and a genuinely-absent stack were indistinguishable on the wire, and ingest likely dropped the
            // former silently. Fixed by dropping frame-less samples here, before interning, rather than
            // inventing a second wire-format sentinel for "empty stack".
            ...
            int? threadIdAttr = null;
            if (threadIdRegistry.TryGetManagedThreadId(sample.OsThreadId, out var managedThreadId))
            {
                threadIdAttr = InternAttribute(dictionary, stringTable, attributeTable, ThreadIdKey, new AnyValue { IntValue = managedThreadId });
            }
            ...
            resolved.Add(new ResolvedSample(stackIndex, threadIdAttr, threadNameAttr, linkIndex));
        }
        ...
        // cpu:nanoseconds, only when a real interval is known -- samples are time-valued, so no period
        // means nothing meaningful to emit. Emit a profile only when at least one sample survived
        // filtering: the OTLP profiles ingest rejects a Profile with zero samples ("no_samples" drop).
        if (periodNanos > 0 && resolved.Count > 0)
        {
            // Per-sample value credit: the caller's overrun/round-robin-corrected value when supplied,
            // otherwise the configured period (see the perSampleValueNanos parameter note).
            var sampleValueNanos = perSampleValueNanos > 0 ? perSampleValueNanos : periodNanos;

            scopeProfiles.Profiles.Add(BuildProfile(dictionary, stringTable, startUnixNano, durationNano, periodNanos,
                CpuSampleTypeName, NanosecondsUnit, PeriodTypeName, resolved, valueForSample: _ => sampleValueNanos, includeSample: _ => true));
        }
    ...
    private readonly struct ResolvedSample
    {
        ...
        public ResolvedSample(int stackIndex, int? threadIdAttr, int threadNameAttr, int linkIndex)
        {
        ...
        string periodTypeName, List<ResolvedSample> resolved, System.Func<ResolvedSample, long> valueForSample, System.Func<ResolvedSample, bool> includeSample)
    {
        ...
        foreach (var identity in order)
        {
            ...
            if (identity.ThreadIdAttr.HasValue)
            {
                protoSample.AttributeIndices.Add(identity.ThreadIdAttr.Value);
            }
