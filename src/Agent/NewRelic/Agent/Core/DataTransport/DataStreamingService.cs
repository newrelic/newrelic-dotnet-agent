        ...
        private async Task<int> WaitForResponse()
        {
            ...
            catch (RpcException rpcEx)
            {
                ...
                if (Log.IsEnabledFor(logLevel))
                {
                    ...
                    {
                        Log.LogMessage(logLevel, $"consumer {ConsumerID} - gRPC RpcException encountered marking the response stream as cancelled. This occurs when a stream has been inactive for period of time.  A new stream will be created when needed. {rpcEx.Status}");
                    }
                    else
                    {
                        Log.LogMessage(logLevel, $"consumer {ConsumerID} - gRPC RpcException encountered while handling gRPC server responses: {rpcEx.Status}");
                    }
            ...
            catch (Exception ex)
            {
                ...
                if (Log.IsEnabledFor(logLevel))
                {
                    Log.LogMessage(logLevel, $"consumer {ConsumerID} - Unknown exception encountered while handling gRPC server responses: {ex}");
                }
            }
