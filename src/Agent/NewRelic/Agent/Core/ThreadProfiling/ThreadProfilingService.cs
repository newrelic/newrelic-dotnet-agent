...
public class ThreadProfilingService : ConfigurationBasedService, IThreadProfilingSessionControl, IThreadProfilingProcessing, ISampleSink, IThreadProfilingStatus
{
    ...
    public ThreadProfilingService(IDataTransportService dataTransportService, INativeMethods nativeMethods, int maxAggregatedNodes = 20000, IContinuousProfilingSessionControl continuousProfilingSessionControl = null, IThreadProfilingSampler sampler = null, IAgentHealthReporter agentHealthReporter = null)
    {
    ...
    public void Stop()
    {
        ...
        //
        // This is a deliberate, reviewed trade-off, not an oversight: a stalled CLR exit affects every
        // request in flight and the host's restart/recycle time, while the cost of suppression is losing
        // one session's already-collected-but-unsent profile data. We accept the latter to bound the
        // former. The discarded-data side of that trade-off is made visible below via a supportability
        // metric and log line so it isn't silent.
        var sessionWasRunning = IsThreadProfilingActive;
        var inFlightSessionId = _profileSessionId;

        StopThreadProfilingSession(inFlightSessionId, reportData: false);

        if (sessionWasRunning)
        {
            Log.Info("Thread profiling session {0} was still in flight at agent shutdown; its collected profile data is being discarded rather than sent, to avoid stalling CLR exit on a collector POST.", inFlightSessionId);
            _agentHealthReporter?.ReportSupportabilityCountMetric(MetricNames.SupportabilityThreadProfilingShutdownDataDiscarded);
        }
    }
    ...
    public bool StartThreadProfilingSession(int profileSessionId, uint frequencyInMsec, uint durationInMsec)
    {
        ...
        using (ProfilingMutualExclusionGate.Acquire())
        {
            ...
            try
            {
                ...
                // If a prior Stop() timed out its bounded join, its sampling worker can still be winding
                // down -- and that worker is still reading the tree/name cache/pruning list and
                // _profileSessionId in its end-of-session aggregation. The sampler's own CompareExchange
                // guard would refuse to arm in that case anyway (Start returns false), but the ResetCache()
                // below would first wipe that shared state out from under the lingering worker, silently
                // corrupting or emptying the in-flight profile. Refuse to start cleanly instead -- matching
                // the "Start() returns false" contract -- without touching the state the worker needs.
                if (_sampler.IsRunning)
                {
                    Log.Info("Thread profiling start refused: a prior sampling worker is still winding down.");
                    return false;
                }
    ...
    public bool StopThreadProfilingSession(int profileId, bool reportData = true)
    {
        ...
        // Stop() waits (bounded) for the sampling worker to wind down and reports whether it is actually
        // stopped. Only when it is confirmed stopped is it safe to reset _profileSessionId and the
        // tree/name cache -- otherwise a still-running worker's PerformAggregation is concurrently reading
        // exactly those structures (SortPruningTree/SerializeData), and clearing them mid-flight would
        // silently drop the profile or POST an empty-but-well-formed one (zero samples/threads,
        // profile_id: 0). A timed-out worker clears _profileSessionId itself in PerformAggregation, and
        // the next StartThreadProfilingSession refuses to arm (and to ResetCache) until that worker is
        // gone, so leaving the state intact here is safe.
        var workerStopped = _sampler.Stop();
