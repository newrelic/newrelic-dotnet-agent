...
public class Scheduler : IScheduler, IDisposable
{
    ...
    private static Timer PrivateCreateExecuteOnceTimer(Action action, TimeSpan timeUntilExecution, bool trackAsAgentWork)
    {
        var ignoreWorkAction = new TimerCallback(_ =>
        {
            ...
            // SetAgentWork lives INSIDE the try so its ResetAgentWork in the finally is structurally
            // paired with it: a throwing IContinuousProfilingContext.SetAgentWork can no longer orphan
            // the native depth-counter increment (the reset always runs) nor escape unhandled onto this
            // ThreadPool thread (the catch logs it). See CaptureAgentWorkContext for why an orphaned
            // increment permanently pins the thread's slot.
            try
            {
            ...
            {
                Log.Error(exception, "CreateExecuteOnceTimer() failed");
            }
            finally
            {
    ...
    public static Timer CreateExecuteEveryTimer(Action action, TimeSpan timeBetweenExecutions, TimeSpan? optionalInitialDelay = null, bool trackAsAgentWork = true)
    {
        ...
        var ignoreWorkAction = new TimerCallback(_ =>
        {
            ...
            // paired with it: a throwing IContinuousProfilingContext.SetAgentWork can no longer orphan
            // the native depth-counter increment (the reset always runs) nor escape unhandled onto this
            // ThreadPool thread (the catch logs it).
            try
            {
                continuousProfilingContext?.SetAgentWork();
