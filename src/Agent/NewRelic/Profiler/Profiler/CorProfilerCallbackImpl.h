...
#pragma warning(pop)
        ...
        public:
            CInstrumentationMethod() : _referenceCount(0)
            {
                _systemCalls = std::make_shared<SystemCalls>();
                GetSingletonish() = this;

                m_pProfiler = std::make_unique<CorProfilerCallbackImpl>();
                m_pProfiler->AddRef();
            }
            ~CInstrumentationMethod()
            {
            ...
            virtual HRESULT STDMETHODCALLTYPE IInstrumentationMethod::Initialize(IProfilerManager* pProfilerManager) override
            {
                ...
                {
                    LogError(_X("Error initializing CLR profiler info from IProfilerManager: "), hrGetCorProfilerInfo);
                    return CORPROF_E_PROFILER_CANCEL_ACTIVATION;
                }
        ...
        protected:
            ...
            // there is only one by convention, but that is not guaranteed anywhere
            static CInstrumentationMethod*& GetSingletonish()
            {
                static CInstrumentationMethod* s_profiler = nullptr;
                return s_profiler;
            }
