...
namespace NewRelic { namespace Profiler { namespace ContinuousProfiler
{
    ...
    public:
        ...
        // Concurrency stress for the seqlock. Improved over the earlier fixed 80ms wall-clock window, which
        // admitted it did not reliably force a torn read: writers are now barrier-aligned to start in
        // lockstep, run a HIGH, fixed iteration count (deterministic amount of work, not a timing window),
        // and -- critically -- each write uses a DISTINCT, monotonically increasing value in all three
        // fields. Distinct-per-write values are what give a torn read teeth: any read that splices two
        // different writes' fields (or a write and the intervening Reset's zeros) yields three fields that
        // are no longer all-equal, which the reader flags. A strong-memory-model host (x86/x64) still cannot
        // be forced to tear deterministically -- the CPU does not reorder these loads/stores -- but the tight
        // lockstep plus volume maximizes the interleaving, and this is precisely the test that would go flaky
        // on a weak model (arm64) if WriteSlot's release fence (the one that "exists specifically for weak
        // memory models") or the reader's acquire fence were removed. We do NOT pierce encapsulation to force
        // a torn read; we assert the OBSERVABLE seqlock contract: a successful TryGet never returns spliced
        // fields, and nothing crashes or hangs.
        TEST_METHOD(concurrent_set_reset_get_stays_consistent)
        {
            ...
            std::atomic<int> writersRemaining{ writerCount };
            ...
            for (int t = 0; t < writerCount; ++t)
            {
                ...
                threads.emplace_back([&map, &ready, &go, &writersRemaining, id, iterations]
                {
                    ready.fetch_add(1, std::memory_order_relaxed);
                    while (!go.load(std::memory_order_acquire)) { std::this_thread::yield(); }

                    // Distinct per-write values: a read splicing two iterations' fields is detectable because
                    // the three fields would no longer be equal. Start at 1 so a live write's value is never
                    // the 0 that Reset writes -- keeps "no context" distinct from a real store.
            ...
            // Reader stands in for the suspend-window sampler: loops until every writer has exhausted its
            // fixed iteration budget. Any accepted read whose three fields are not all equal is a spliced
            // (torn) read -- the seqlock must make that impossible.
            threads.emplace_back([&map, &ready, &go, &writersRemaining, &inconsistent, &ids]
            {
                ready.fetch_add(1, std::memory_order_relaxed);
                while (!go.load(std::memory_order_acquire)) { std::this_thread::yield(); }
            ...
            const int totalThreads = writerCount + 1;
            while (ready.load(std::memory_order_relaxed) < totalThreads) { std::this_thread::yield(); }
        ...
        // The compaction pass's only view of live state. Read-only by construction: it must never write
        // a slot, since it runs on the sampling thread against slots owned by app threads.
        TEST_METHOD(snapshot_live_span_ids_returns_every_live_span)
        {
            TraceContextMap map;
            map.Set(static_cast<ThreadID>(0x1000), 1, 2, 111);
            map.Set(static_cast<ThreadID>(0x2000), 3, 4, 222);

            int64_t buffer[8] = {};
            size_t count = 0;
            Assert::IsTrue(map.SnapshotLiveSpanIds(buffer, 8, count));

            Assert::AreEqual(static_cast<size_t>(2), count);
            const bool sawFirst = (buffer[0] == 111 || buffer[1] == 111);
            const bool sawSecond = (buffer[0] == 222 || buffer[1] == 222);
            Assert::IsTrue(sawFirst);
            Assert::IsTrue(sawSecond);
        }

        TEST_METHOD(snapshot_live_span_ids_skips_reset_and_zero_span_slots)
        {
            TraceContextMap map;
            const ThreadID reset = static_cast<ThreadID>(0x1000);
            map.Set(reset, 1, 2, 111);
            map.Reset(reset);
            map.Set(static_cast<ThreadID>(0x2000), 3, 4, 0);
            map.Set(static_cast<ThreadID>(0x3000), 5, 6, 333);
            ...
            Assert::AreEqual(static_cast<int64_t>(333), buffer[0]);
        }

        // A snapshot that cannot fit every live span must FAIL, so the caller declines to reclaim
        // anything rather than reclaiming an entry it merely failed to observe.
        TEST_METHOD(snapshot_live_span_ids_fails_when_the_buffer_is_too_small)
        {
            TraceContextMap map;
            map.Set(static_cast<ThreadID>(0x1000), 1, 2, 111);
            map.Set(static_cast<ThreadID>(0x2000), 3, 4, 222);
            ...
            Assert::IsFalse(map.SnapshotLiveSpanIds(buffer, 1, count));
        }

        // A slot from a previous profiling session is unreadable by TryGet and so can never produce a
        // link -- but the snapshot still reports it, deliberately: over-reporting only delays a
        // reclamation, while under-reporting could reclaim an entry a slot still holds.
        TEST_METHOD(snapshot_live_span_ids_includes_a_previous_generation_slot)
        {
            TraceContextMap map;
            map.Set(static_cast<ThreadID>(0x1000), 1, 2, 111);
            map.NewGeneration();
            ...
            Assert::AreEqual(static_cast<size_t>(1), count);
            Assert::AreEqual(static_cast<int64_t>(111), buffer[0]);
        }

        // A slot parked mid-write must fail the WHOLE snapshot, not be silently omitted: an omitted
        // live span is exactly the false positive compaction must never be handed. This is the only
        // reachable failure branch in SnapshotLiveSpanIds -- the capacity branch cannot fire in
        // production, where the caller's buffer is sized to SlotCount -- and it is what exercises
        // SnapshotReadAttempts running out.
        TEST_METHOD(snapshot_live_span_ids_fails_on_a_slot_that_stays_mid_write)
        {
            TraceContextMap map;
            const ThreadID thread = static_cast<ThreadID>(0x1000);
            map.Set(thread, 1, 2, 111);

            // Friend access, as the probing tests already use. Leaving Seq odd is exactly the state a
            // thread suspended inside WriteSlot leaves behind, and no retry can clear it.
            ...
            int64_t buffer[8] = {};
            size_t count = 0;
            Assert::IsFalse(map.SnapshotLiveSpanIds(buffer, 8, count));
        }
    };
}}}
