...
// Minimal ICorProfilerInfo stand-in for tests that only need ContinuousProfiler::Init() to have been
...
// actually calls on a non-sampling path: QueryInterface and InitializeCurrentThread.
//
// The class name is kept for historical continuity, but it now implements the FULL ICorProfilerInfo10
// surface (SuspendRuntime/ResumeRuntime included) so that subclasses -- notably the scriptable
// suspend/resume stub in RichStubCorProfilerInfo4.h -- can opt into handing out ICorProfilerInfo10 from
// their own QueryInterface and drive ContinuousProfiler's CoreCLR stop-the-world path. This base class's
// OWN QueryInterface still deliberately REFUSES ICorProfilerInfo10 (returns E_NOINTERFACE): the lifecycle
// tests rely on that to make the profiler take its "runtime cannot suspend / never samples" branch. The
// SuspendRuntime/ResumeRuntime methods here are plain overridable virtuals that E_NOTIMPL by default; only
// the scriptable subclass makes them return-code- and call-count-controllable.
//
...
// name-stripping ~100 signatures that must otherwise stay verbatim copies of the interface.
#pragma warning(push)
#pragma warning(disable: 4100)

namespace NewRelic { namespace Profiler { namespace ContinuousProfiler
{
    class StubCorProfilerInfo4 : public ICorProfilerInfo10
    {
    public:
        virtual HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void** ppvObject) override
        {
            ...
            // NOTE: ICorProfilerInfo10 (and 5-9) are deliberately refused here even though this class now
            // implements them -- the lifecycle tests depend on the profiler taking its "runtime cannot
            // suspend" branch. The scriptable subclass overrides QueryInterface to hand Info10 out.
        ...
        // ICorProfilerInfo5
        virtual HRESULT STDMETHODCALLTYPE GetEventMask2(DWORD *pdwEventsLow, DWORD *pdwEventsHigh) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE SetEventMask2(DWORD dwEventsLow, DWORD dwEventsHigh) override { return E_NOTIMPL; }

        // ICorProfilerInfo6
        virtual HRESULT STDMETHODCALLTYPE EnumNgenModuleMethodsInliningThisMethod(ModuleID inlinersModuleId, ModuleID inlineeModuleId, mdMethodDef inlineeMethodId, BOOL *incompleteData, ICorProfilerMethodEnum **ppEnum) override { return E_NOTIMPL; }

        // ICorProfilerInfo7
        virtual HRESULT STDMETHODCALLTYPE ApplyMetaData(ModuleID moduleId) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetInMemorySymbolsLength(ModuleID moduleId, DWORD *pCountSymbolBytes) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE ReadInMemorySymbols(ModuleID moduleId, DWORD symbolsReadOffset, BYTE *pSymbolBytes, DWORD countSymbolBytes, DWORD *pCountSymbolBytesRead) override { return E_NOTIMPL; }

        // ICorProfilerInfo8
        virtual HRESULT STDMETHODCALLTYPE IsFunctionDynamic(FunctionID functionId, BOOL *isDynamic) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetFunctionFromIP3(LPCBYTE ip, FunctionID *functionId, ReJITID *pReJitId) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetDynamicFunctionInfo(FunctionID functionId, ModuleID *moduleId, PCCOR_SIGNATURE *ppvSig, ULONG *pbSig, ULONG cchName, ULONG *pcchName, WCHAR wszName[ ]) override { return E_NOTIMPL; }

        // ICorProfilerInfo9
        virtual HRESULT STDMETHODCALLTYPE GetNativeCodeStartAddresses(FunctionID functionID, ReJITID reJitId, ULONG32 cCodeStartAddresses, ULONG32 *pcCodeStartAddresses, UINT_PTR codeStartAddresses[ ]) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetILToNativeMapping3(UINT_PTR pNativeCodeStartAddress, ULONG32 cMap, ULONG32 *pcMap, COR_DEBUG_IL_TO_NATIVE_MAP map[ ]) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetCodeInfo4(UINT_PTR pNativeCodeStartAddress, ULONG32 cCodeInfos, ULONG32 *pcCodeInfos, COR_PRF_CODE_INFO codeInfos[ ]) override { return E_NOTIMPL; }

        // ICorProfilerInfo10
        virtual HRESULT STDMETHODCALLTYPE EnumerateObjectReferences(ObjectID objectId, ObjectReferenceCallback callback, void *clientData) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE IsFrozenObject(ObjectID objectId, BOOL *pbFrozen) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE GetLOHObjectSizeThreshold(DWORD *pThreshold) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE RequestReJITWithInliners(DWORD dwRejitFlags, ULONG cFunctions, ModuleID moduleIds[ ], mdMethodDef methodIds[ ]) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE SuspendRuntime(void) override { return E_NOTIMPL; }
        virtual HRESULT STDMETHODCALLTYPE ResumeRuntime(void) override { return E_NOTIMPL; }
    };
}}}
