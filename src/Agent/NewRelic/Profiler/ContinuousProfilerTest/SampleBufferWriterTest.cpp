...
namespace NewRelic { namespace Profiler { namespace ContinuousProfiler
{
    ...
    public:
        ...
        TEST_METHOD(write_start_batch_is_big_endian)
        {
            ...
            Assert::AreEqual(6, static_cast<int>(buffer[1]));    // BatchVersion
        ...
        // BatchStats encodes int64 micros + seven big-endian int32s in field order: threadCount, frameCount,
        // skipped, the v4 additions deferredThreadCount + actualSamplePeriodMs, and the v5 additions
        // truncatedThreadCount + droppedTickCount. Every field is APPENDED after skipped -- exactly where
        // BufferParser reads them when version >= 4 / >= 5.
        TEST_METHOD(write_batch_stats_is_big_endian_in_field_order)
        {
            std::vector<uint8_t> buffer;
            SampleBufferWriter writer(buffer, 1024);

            writer.WriteBatchStats(0x1122334455667788LL, 0x01020304, 0x0A0B0C0D, 0x00000005, 0x00000300, 0x000004D2, 0x00000007, 0x00000009);
            ...
            const uint8_t expected[36] = {
                ...
                0x00, 0x00, 0x03, 0x00,                         // deferredThreadCount (int32) == 768
                0x00, 0x00, 0x04, 0xD2,                         // actualSamplePeriodMs (int32) == 1234
                0x00, 0x00, 0x00, 0x07,                         // truncatedThreadCount (int32) == 7 (v5)
                0x00, 0x00, 0x00, 0x09,                         // droppedTickCount (int32) == 9 (v5)
            };
            for (int i = 0; i < 36; ++i)
            {
        ...
        // EstimateFrameBytes charges a not-yet-interned frame the full inline definition (2-byte code +
        // 2-byte length prefix + UTF-16LE bytes) but an already-interned frame only its 2-byte
        // back-reference -- mirroring WriteCodedFrameString. This is the fix that stops the mid-batch
        // overflow guard from charging shared framework frames as fresh definitions and truncating early.
        TEST_METHOD(estimate_frame_bytes_charges_interned_frames_as_back_references)
        {
            std::vector<uint8_t> buffer;
            SampleBufferWriter writer(buffer, 1024);
            writer.BeginBatch();

            const xstring_t frame = _X("System.Threading.Foo"); // 20 chars
            const size_t fullCost = 2 + 2 + (20 * 2); // code + length prefix + UTF-16LE bytes

            // Not yet seen -> full definition cost.
            Assert::AreEqual(fullCost, writer.EstimateFrameBytes(frame));

            // After a first write it is interned -> a second sight costs only the 2-byte back-reference.
            writer.WriteCodedFrameString(frame);
            Assert::AreEqual(static_cast<size_t>(2), writer.EstimateFrameBytes(frame));
        }

        // Frame strings longer than MaxStringChars are charged at the capped length, never the raw size --
        // the estimate must track WriteString's own truncation so it stays a true upper bound.
        TEST_METHOD(estimate_frame_bytes_caps_at_max_string_chars)
        {
            std::vector<uint8_t> buffer;
            SampleBufferWriter writer(buffer, 4096);
            writer.BeginBatch();

            const xstring_t longFrame(600, _X('x'));
            const size_t cappedCost = 2 + 2 + (SampleBufferWriter::MaxStringChars * 2);
            Assert::AreEqual(cappedCost, writer.EstimateFrameBytes(longFrame));
        }
        ...
        TEST_METHOD(will_fit_refuses_an_oversized_sample_without_corrupting_the_buffer)
        {
            ...
            const uint8_t expectedStartBatch[10] = { 0x01, 0x06, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 };
            for (int i = 0; i < 10; ++i)
            {
        ...
        TEST_METHOD(producer_truncates_mid_batch_and_never_overruns_max_bytes)
        {
            ...
            // trailer(38) = 126 fits, but a 3rd sample would need another 39 and does not.
            ...
            // Reserve enough for WriteBatchStats (1 + 8 + 4*7 = 37) + WriteEndBatch (1) = 38 (v5 trailer).
            const size_t trailerBytes = 38;
            ...
            writer.WriteBatchStats(0, emitted, 0, 0, 0, 0, 0, 0);
            writer.WriteEndBatch();
