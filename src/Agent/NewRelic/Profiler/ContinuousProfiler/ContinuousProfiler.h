...
namespace NewRelic { namespace Profiler { namespace ContinuousProfiler
{
    ...
    public:
        ...
        // Returns S_OK on success (worker running and sampling armed), or an HRESULT failure the managed
        // ContinuousProfilingService treats like a thrown P/Invoke exception (log, unwind, do not arm the
        // drain schedule -- see StartLocked).
        HRESULT Start(uint32_t intervalMs) noexcept
        {
            ...
            //
            // Declared BEFORE the try (not inside it) so it is held continuously across BOTH the happy
            // path AND the catch handler's failure cleanup. A lock scoped inside the try would be released
            // by stack unwinding the instant std::thread construction throws -- BEFORE control reaches the
            // catch -- so the catch's _samplingActive reset would then run with NO lifecycle lock held. A
            // concurrent Start() on another thread could acquire the now-free mutex in that gap, spawn a
            // worker, and set _samplingActive=true, only for this thread's late catch to clobber it back to
            // false: the successful start is silently disarmed (worker alive, flag says inactive) -- the
            // exact "armed-but-dead" outage this serialization exists to prevent, reached from the failure
            // path. The general invariant: a lock meant to serialize a lifecycle operation must be held for
            // the ENTIRE operation, its failure-cleanup path included, not just the happy-path portion.
            ...
            try
            {
                ...
                // A fresh start retires the previous session's agent-work depths. SetAgentWork/ResetAgentWork
                // drive a per-thread nesting-DEPTH counter that is never tombstoned, so an increment orphaned
                // by a managed-side lifecycle race would otherwise keep that thread tagged as agent work --
                // and its samples filtered out of the profile -- for the rest of the process. Bumping the
                // generation here, BEFORE sampling is armed, invalidates every prior-session depth at once and
                // bounds any such leak to the session that caused it. See AgentWorkMap::NewGeneration for why
                // this touches no slot and is safe against a concurrent suspended-runtime reader.
                _agentWork.NewGeneration();

                // Allocate the retired-span table on first start. Deliberately lazy: a process that never
                // enables continuous profiling must not pay for it, matching the managed side's lazy drain
                // buffer. Idempotent across restarts, and the contents are deliberately KEPT across a
                // stop/start -- unlike a per-thread slot, a retirement records a fact about a transaction
                // that really ended, which stays true forever, so there is no generation concept here.
                if (!_retiredSpans.EnsureAllocated())
                {
                    LogWarn(L"CP: could not allocate the retired-span registry; samples may be linked to "
                        L"already-ended spans until the next successful start. "
                        L"** THIS MAY RESULT IN INCORRECT TRACE CORRELATION **");
                }

                // Allocate the pending-push cells on the same terms. A failure here is strictly less severe
                // than the registry's: it only widens the residual window compaction cannot see into (see
                // PendingPushMap.h), rather than disabling rejection outright.
                if (!_pendingPushes.EnsureAllocated())
                {
                    LogWarn(L"CP: could not allocate the pending-push cell table; the compaction pass "
                        L"cannot observe in-flight trace-context pushes, so a sample may briefly be linked "
                        L"to an already-ended span. ** THIS MAY RESULT IN INCORRECT TRACE CORRELATION **");
                }
                ...
                // Publish under _mtx_wake and notify so the flag change is visible to the worker's wait
                // predicate and the notification cannot be lost against a worker between evaluating that
                // predicate and blocking. Since CompactRetiredSpansIfDue the paused wait has a
                // CompactionIntervalMs timeout, so a lost signal here delays the resume by up to that
                // interval rather than stranding it -- but do NOT weaken this to an unlocked store on
                // that basis: a resume silently deferred by seconds is a real gap in collection.
                {
                ...
                if (!_workerThread.joinable())
                {
                    ...
                    // Test seam (default-inert): drive Start()'s std::thread-creation failure path
                    // deterministically, without having to actually exhaust OS thread resources. One-shot
                    // -- the exchange clears it as it throws, so only the immediately-following Start()
                    // fails. See SetFailWorkerCreationForTesting.
                    if (_failWorkerCreationForTesting.exchange(false))
                    {
                        throw std::runtime_error("CP: forced worker-thread creation failure (test seam)");
                    }
            ...
            {
                // std::thread creation can throw std::system_error (e.g. resource exhaustion). Without
                // this, Start() would leave _samplingActive=true with no worker thread -- armed-but-dead
                // and silent, with CP collecting nothing until some later Start(). Reset the flag so the
                // session is cleanly stopped (a subsequent Start() re-arms and respawns the worker) and
                // log so the outage is operator-visible instead of a silent gap.
                //
                // This cleanup runs while _mtx_lifecycle is STILL held (it was acquired before the try and
                // is not released until this catch returns), so it is serialized against a concurrent
                // Start()/Stop()/Shutdown() exactly like the happy path is. A racing Start() that acquired
                // _mtx_lifecycle first would have fully spawned its worker and released the lock before we
                // even enter here; one that has not yet acquired it cannot until we return -- so this store
                // can never clobber another caller's successful start. Lock order stays _mtx_lifecycle ->
                // _mtx_wake, matching Stop()/Shutdown(); no inversion is introduced.

                // Test seam (default-inert): observe the lock state during this cleanup -- on the failing
                // thread, while _mtx_lifecycle is still held -- BEFORE the flag reset below. See
                // SetOnStartCatchForTesting.
                if (_onStartCatchForTesting)
                {
                    _onStartCatchForTesting();
                }
                ...
                LogError(L"CP: failed to start sampling thread: ", ex.what());

                // The session was cleanly rolled back above; tell managed code to unwind rather than arm
                // a drain schedule over a sampler that isn't running.
                return E_FAIL;
            }
        }
        ...
        // Returns S_OK on success, or E_FAIL if freeing the session buffers threw (logged and swallowed
        // as before -- the return simply carries that fact to managed code now that the ABI can).
        HRESULT Stop() noexcept
        {
            ...
            // thread (which Stop does NOT join -- it only parks it) rests on three facts:
            ...
            //      out of CaptureAllThreads and cannot touch any buffer ReleaseSamplingResources frees; and
            //   3. SuspendMutex does NOT by itself stop a NEW capture from starting after this free -- a
            //      worker that already passed its _samplingActive check but had not yet taken SuspendMutex
            //      would acquire it once we release and re-allocate the buffers we just freed. That
            //      check-then-act race is closed on the worker's side: CaptureAllThreads re-reads
            //      _samplingActive after taking SuspendMutex and bails when it is false (which fact 1
            //      guarantees it will observe, since our store happens-before our SuspendMutex release
            //      happens-before the worker's later acquire). Fact 2 alone only prevents a free DURING a
            //      capture; fact 3 is what prevents a capture STARTING after the free.
        ...
        // Returns S_OK on success, or E_FAIL if teardown threw (logged and swallowed as before). The
        // destructor's safety-net call ignores the result; managed Dispose logs it.
        ...
        // These four seam methods can't meaningfully fail (a lock-free map write), but return HRESULT so
        // the whole CP ABI is uniform rather than a mixed void/HRESULT surface.
        HRESULT SetTraceContext(int64_t traceIdHigh, int64_t traceIdLow, int64_t spanId) noexcept
        {
        ...
        HRESULT ResetTraceContext() noexcept
        {
            _traceContexts.Reset(CurrentManagedThreadId());
            return S_OK;
        }

        // Record that every span id in `spanIds` has ENDED, so no later sample can be linked to it.
        // Called once per terminating transaction, with that transaction's already-materialized span ids,
        // from whatever thread terminated it -- the completing app thread, or the GC FINALIZER THREAD for
        // a transaction that was reaped rather than ended cleanly. So: never blocks, never throws, never
        // allocates, and takes no lock any app thread could be holding while suspended.
        //
        // This is the cross-thread half of the fix. ResetTraceContext (above) stays and is still the cheap
        // common case -- a segment ending on the same thread that pushed it clears that thread's own slot
        // directly. RetireSpans covers what ResetTraceContext structurally cannot: the pushing thread
        // never runs any completion code, so its slot is never cleared and keeps linking samples to a
        // finished transaction for as long as the thread stays idle. The two are complementary.
        //
        // Returns S_OK even when an individual insert was dropped: a dropped retirement is warned about
        // and counted here rather than surfaced to managed code, which has no useful recovery. Overflow is
        // never silent.
        HRESULT RetireSpans(const int64_t* spanIds, int32_t count) noexcept
        {
            if (count <= 0)
            {
                return S_OK; // nothing to retire (a transaction with no materialized spans).
            }
            ...
            {
                _retiredSpans.TryInsert(spanIds[i]);
            }

            ReportRetiredSpanOverflowIfNeeded();
            return S_OK;
        }

        // Hand the CALLING managed thread the address of its pending-push cell: the one place it can
        // publish "I am about to push span S" before it starts the push, so a compaction pass can see the
        // intent that no TraceContextMap slot yet reflects. See PendingPushMap.h for why this is a separate
        // map keyed by ThreadID rather than a field in a TraceContextMap slot -- a cached pointer into a
        // slot would dangle into a DIFFERENT thread's slot once the original slot is tombstoned and
        // reclaimed, turning every later write into a foreign write.
        //
        // Expected to be called ONCE per managed thread; the returned address stays valid until that thread
        // exits (ThreadDestroyed -> Forget). A failure is benign: the caller pushes without publishing its
        // intent, which is the pre-cell behaviour, so it is reported but not warned about on every thread.
        HRESULT GetPendingPushCell(void** cellOut) noexcept
        {
            if (cellOut == nullptr)
            {
                return E_POINTER;
            ...
            *cellOut = nullptr;
            ...
            int64_t* cell = _pendingPushes.CellFor(CurrentManagedThreadId());
            if (cell == nullptr)
            {
                return E_FAIL; // table unallocated, probe budget exhausted, or no resolvable ThreadID.
            }
        ...
        HRESULT ResetAgentWork() noexcept
        {
        ...
        //
        // Reset/Forget below are therefore cross-thread calls into maps documented as
        // single-writer-per-slot; they rely on this method running only once the target thread has
        // already exited (see TraceContextMap::Reset / AgentWorkMap::Forget for the CLR guarantee this
        // is based on). If ThreadDestroyed is ever deferred, dispatched off this callback thread, or
        // invoked speculatively before the thread fully exits, that invariant -- and this method's
        // safety -- would need to be re-examined.
        void ThreadDestroyed(ThreadID threadId) noexcept
        {
            _traceContexts.Reset(threadId);
            _agentWork.Forget(threadId);
            // Releases the dead thread's pending-push cell, so a later thread reusing the recycled
            // ThreadID cannot inherit a stale in-flight span id (which would pin that span's registry
            // entry forever) and so the fixed table does not fill over a process's thread-pool churn.
            _pendingPushes.Forget(threadId);
        }

        // A CLR unload (collectible ALC, module, or class) frees the MethodDesc*/Module* allocations
        // _nameCache is keyed by raw address of, and the allocator can reissue those addresses to new code
        // -- a stale cache hit would then report the wrong name forever. Called from arbitrary CLR unload
        // callback threads, so this only flags a pending invalidation rather than touching the not-thread-
        // safe, worker-owned _nameCache directly; the worker clears it at the top of its next capture tick
        // (see CaptureAllThreads). Always a full clear (no cheap way to drop just one module's entries) --
        ...
        // Test seam: whether sampling is currently armed. Lets a lifecycle test assert the "armed-but-dead"
        // invariant -- a live worker must never coexist with _samplingActive==false.
        bool IsSamplingActiveForTesting() const noexcept
        {
            return _samplingActive.load();
        }

        // Test seam: force the NEXT worker-thread creation inside Start() to throw, so a lifecycle test can
        // drive Start()'s failure/catch path deterministically without actually exhausting OS thread
        // resources. One-shot -- Start() clears it as it throws, so only the immediately-following Start()
        // fails.
        void SetFailWorkerCreationForTesting() noexcept
        {
            _failWorkerCreationForTesting.store(true);
        }

        // Test seam: callback invoked at the very top of Start()'s catch handler -- on the failing thread,
        // while _mtx_lifecycle is still held, and BEFORE the _samplingActive failure reset. Lets a lifecycle
        // test observe, from a PROBE thread, whether that failure cleanup runs under the lifecycle lock
        // (a probe's try_lock should fail while the lock is held across the whole try/catch).
        void SetOnStartCatchForTesting(std::function<void()> hook)
        {
            _onStartCatchForTesting = std::move(hook);
        }

        // Test seam: install a callback fired PARTWAY through the per-session reservation block in
        // CaptureAllThreads -- after the _capture temp is built but before the _resolved temp -- so a test
        // can throw there (modeling a bad_alloc on a memory-capped container) to assert the all-or-nothing
        // invariant: the member vectors are left untouched on a mid-block failure, and a subsequent tick
        // retries and completes the full reservation. Pass nullptr to clear.
        void SetReservationFailpointForTesting(std::function<void()> hook)
        {
            _reservationFailpointForTesting = std::move(hook);
        }

        // Test seam: install a callback fired INSIDE the stop-the-world window in CaptureAllThreads --
        // after SuspendRuntime succeeds, in the try block that wraps ProfileAllThreads -- so a test can
        // throw there (modeling a capture that faults while the runtime is suspended) to assert the
        // suspend/resume invariant under failure: the catch(...) swallows it AND ResumeRuntime still runs,
        // so an exception in the window can never leave the runtime permanently suspended (a full-process
        // hang on the next tick). Only reached on the CoreCLR suspend path. Pass nullptr to clear.
        void SetCaptureWindowFailpointForTesting(std::function<void()> hook)
        {
            _captureWindowFailpointForTesting = std::move(hook);
        }

        // Test seam: current slot counts of the two per-session capture vectors. Lets a test prove a
        // mid-block reservation failure left BOTH untouched (still 0), never a torn
        // _capture==ThreadCountForReservation / _resolved==0 state. Read on the test thread only (no worker
        // running), so the plain reads are race-free.
        size_t CaptureSlotCountForTesting() const noexcept
        {
            return _capture.size();
        }

        size_t ResolvedSlotCountForTesting() const noexcept
        {
            return _resolved.size();
        }

        // Test seam: whether the per-session reservation is fully complete AND internally consistent --
        // both vectors sized to ThreadCountForReservation and every per-slot FunctionIds/Frames buffer
        // reserved to at least MaxStackFramesSupported (so no push_back can reallocate inside the suspend
        // window). Lets a test assert a retry tick after a failed one finishes the whole block.
        bool ReservationIsFullyCompleteForTesting() const noexcept
        {
            if (_capture.size() != ThreadCountForReservation || _resolved.size() != ThreadCountForReservation)
            {
                return false;
            }
            for (const auto& slot : _capture)
            {
                if (slot.FunctionIds.capacity() < MaxStackFramesSupported)
                {
                    return false;
                }
            }
            for (const auto& slot : _resolved)
            {
                if (slot.Frames.capacity() < MaxStackFramesSupported)
                {
                    return false;
                }
            }
        ...
        // Test seam: try to acquire the lifecycle mutex WITHOUT blocking, releasing it immediately if
        // acquired; returns whether the acquisition succeeded. MUST be called from a thread OTHER than any
        // thread already holding _mtx_lifecycle (std::mutex::try_lock on a mutex the caller already owns is
        // undefined behavior). Used by a probe thread to prove Start()'s failure cleanup holds the lock.
        bool TryLockLifecycleForTesting()
        {
            if (_mtx_lifecycle.try_lock())
            {
                _mtx_lifecycle.unlock();
                return true;
            }
            return false;
        }
        ...
        // thread. Arms _samplingActive first (a real capture only runs while sampling is active -- see the
        // TOCTOU re-check in CaptureAllThreads), so this drives the normal active-capture path. Under the
        // lifecycle stub (no ICorProfilerInfo10, isCoreClr == false) it allocates the per-session buffers
        // (_stackwalk, _capture, _resolved, _threadList) and publishes one (empty) batch into the
        // sample-buffer queue, all without ever suspending a runtime -- letting a lifecycle test
        // deterministically drive the allocate path so it can then assert Stop() frees it.
        ...
        // Test seam: reproduce the worker's already-in-flight capture that lost the check-then-act race to
        // Stop(). The real worker tests _samplingActive in SamplingThreadStart, then Stop() flips the flag
        // false and frees the session buffers before the worker's CaptureAllThreads reaches SuspendMutex.
        // This runs one capture pass WITHOUT re-arming sampling, so it stands in for that post-check,
        // post-Stop entry: with the TOCTOU re-check in place CaptureAllThreads observes _samplingActive
        // false under SuspendMutex and bails (no buffer re-allocation, no stale batch published); without
        // the re-check it would re-allocate the just-freed buffers and publish a post-stop batch.
        void CaptureAfterStopRaceForTesting()
        {
            CaptureAllThreads();
        }

        // Test seam: direct access to the name cache so a test can seed entries (as a real capture tick's
        // resolution would) and observe whether an unload signal (InvalidateNameCache) followed by a
        // capture tick actually cleared them. Touched on the test thread only, with no worker running, so
        // the otherwise worker-owned cache is race-free there.
        NameCache& NameCacheForTesting() noexcept
        {
            return _nameCache;
        }
        ...
        // Test seam: drive the real static per-frame snapshot callback (StaticStackFrameCallback) directly,
        // with no CLR, to exercise the MaxStackFramesSupported overflow abort and leaf-vs-root retention
        // (keeps the leaf-most frames on a too-deep stack). `deliveredFrames` are handed to the callback
        // leaf->root as the CLR would during DoStackSnapshot; delivery stops the instant the callback
        // returns non-S_OK, mirroring SWA_ABORT. A local heap StackWalk keeps the 128-slot buffer off the
        // test stack.
        struct StackFrameCallbackResultForTesting
        {
            std::vector<FunctionID> RetainedFunctionIds;
            bool Truncated{ false };
            TraceContext StampedContext{};
            size_t MaxFramesSupported{ MaxStackFramesSupported };
        };

        StackFrameCallbackResultForTesting DriveStackFrameCallbackForTesting(ThreadID managedTID, const std::vector<FunctionID>& deliveredFrames)
        {
            auto stackwalk = std::make_unique<StackWalk>();
            TraceContext contextOut{};
            ThreadProfile profile(managedTID, _corProfilerInfo, _nameCache, *stackwalk, _traceContexts, _retiredSpans, contextOut);

            for (const auto functionId : deliveredFrames)
            {
                if (StaticStackFrameCallback(functionId, 0, 0, 0, nullptr, &profile) != S_OK)
                {
                    // The CLR aborts the walk (SWA_ABORT) the moment the callback returns non-S_OK; stop
                    // delivering frames, just as DoStackSnapshot would.
                    break;
                }
            }

            StackFrameCallbackResultForTesting result{};
            for (auto it = std::begin(profile._stackwalk); it != profile._frameNext; ++it)
            {
                result.RetainedFunctionIds.push_back(it->functionId);
            }
            result.Truncated = profile._truncated;
            result.StampedContext = contextOut;
            return result;
        }

        // Test seam: seed a trace context for a managed thread id, so DriveStackFrameCallbackForTesting can
        // verify StaticStackFrameCallback stamps it on the FIRST callback invocation (the one point in the
        // capture that is guaranteed suspended on every platform). Wraps the same TraceContextMap::Set the
        // real accept/finish path drives.
        void SeedTraceContextForTesting(ThreadID managedTID, int64_t traceIdHigh, int64_t traceIdLow, int64_t spanId) noexcept
        {
            _traceContexts.Set(managedTID, traceIdHigh, traceIdLow, spanId);
        }

        // Test seam: retire span ids exactly as the managed transaction-end path does, so a test drives
        // the real RetireSpans -> RetiredSpanRegistry -> StaticStackFrameCallback path rather than a
        // reconstruction of it. Allocates the table because tests construct a bare profiler without
        // calling Start(), which is what allocates it in production.
        void RetireSpansForTesting(const std::vector<int64_t>& spanIds) noexcept
        {
            _retiredSpans.EnsureAllocated();
            RetireSpans(spanIds.empty() ? nullptr : spanIds.data(), static_cast<int32_t>(spanIds.size()));
        }

        // Test seam: run one compaction pass synchronously, without waiting for the sampling cadence.
        // Reclamation takes TWO consecutive clean passes (see RetiredSpanRegistry::CompactAgainst), so a
        // test that wants an entry actually reclaimed must call this twice.
        void CompactRetiredSpansForTesting() noexcept
        {
            _retiredSpans.CompactAgainst(_traceContexts, _pendingPushes);
        }

        // Test seams: OBSERVE the registry's reclamation state. Read-only -- deliberately not a way to
        // drive compaction, because the test these exist for (a_paused_sampling_worker_still_compacts...)
        // has to prove the WORKER performed the passes on its own cadence. Both underlying counters are
        // atomics, so reading them from the test thread while the worker compacts is race-free.
        uint64_t RetiredSpanReclaimedCountForTesting() const noexcept
        {
            return _retiredSpans.ReclaimedCountForTesting();
        }

        size_t RetiredSpanOccupiedCountForTesting() const noexcept
        {
            return _retiredSpans.OccupiedCountForTesting();
        }

        // Test seam: drive the real ProfileAllThreads sweep against a richer stub CLR that returns a live
        // thread list and scripted DoStackSnapshot behavior per thread (the minimal lifecycle stub
        // E_NOTIMPL's the whole capture pipeline and can only drive an empty thread list). Mirrors the
        // pre-suspend setup CaptureAllThreads does, then enumerates and sweeps, returning every counter
        // ProfileAllThreads produces plus the retained per-thread FunctionID sequences.
        struct ProfileAllThreadsResultForTesting
        {
            ...
            std::vector<std::vector<FunctionID>> RetainedFunctionIds; // one entry per captured (retained) thread
            std::vector<DWORD> CapturedOsThreadIds; // one entry per captured (retained) thread, same order
            size_t MaxFramesSupported{ MaxStackFramesSupported };
        };

        ProfileAllThreadsResultForTesting ProfileAllThreadsForTesting()
        {
            // Mirror the pre-suspend setup CaptureAllThreads does before ProfileAllThreads: allocate the
            // reused walk buffer and size the capture slots. Kept minimal (no _resolved -- ProfileAllThreads
            // never touches it) and deliberately not sharing CaptureAllThreads' all-or-nothing reservation
            // block, so this seam cannot perturb that production path.
            if (!_stackwalk)
            {
            ...
            ProfileAllThreadsResultForTesting result{};
            ProfileAllThreads(result.FailedSnapshotCount, result.OverflowCount, result.ExceptionCount, result.TruncatedStackCount);
            result.CapturedCount = _capturedCount;
            for (size_t i = 0; i < _capturedCount; ++i)
            {
                result.RetainedFunctionIds.push_back(_capture[i].FunctionIds);
                result.CapturedOsThreadIds.push_back(_capture[i].OsThreadId);
            }
            return result;
        }
    ...
    private:
        ...
        struct ThreadProfile
        {
            ...
            // Retired-span filter applied to the value _traceContexts hands back -- see
            // StaticStackFrameCallback. const because the suspend-window path only ever reads it.
            const RetiredSpanRegistry& _retiredSpans;
            ...
            ThreadProfile(ThreadID managedTID, ICorProfilerInfo4* corProfilerInfo, NameCache& nameCache, StackWalk& stackwalk,
                TraceContextMap& traceContexts, const RetiredSpanRegistry& retiredSpans, TraceContext& contextOut) :
                _corProfilerInfo(corProfilerInfo), _nameCache(nameCache), _stackwalk(stackwalk), _frameNext(std::begin(_stackwalk)),
                _managedTID(managedTID), _traceContexts(traceContexts), _retiredSpans(retiredSpans), _contextOut(contextOut)
            {}
        ...
        // How often the retired-span registry is compacted, in milliseconds. A FIXED CONSTANT, and
        // deliberately NOT derived from the configured sampling interval or any other tuning knob:
        // coupling correctness to an orthogonal performance dial is exactly what got the previous
        // TTL-based fix rejected. Compaction only ever reclaims entries it has PROVEN no live slot
        // references, so this cadence affects how much registry capacity is in use -- never whether a
        // link is correct. Shortening it lowers steady-state occupancy; lengthening it raises it. Neither
        // can create a wrong link.
        static constexpr int64_t CompactionIntervalMs = 5000;

        // Reclaim due registry entries. Deliberately NOT gated on sampling being active: a pause stops
        // the compactor but not the retirements. StopLocked's pause disarms the managed context (so
        // retirements stop with it), but the SEND-BACKOFF pause (ContinuousProfilingService's
        // TripBackoffAndScheduleProbeLocked) stops native sampling WITHOUT disarming it -- so retirements
        // keep arriving with nothing reclaiming them, filling the table and firing the overflow warning
        // for a window in which no sample is taken and no link can be wrong. Compacting while paused is
        // the same off-suspend, single-threaded pass it always was; only the tick that drives it changes.
        //
        // Why a paused pass is no less safe than an active one. NOT because slots hold still: app threads
        // keep writing TraceContextMap slots during a backoff pause, since SetTraceContext has no
        // _samplingActive gate and that pause does not disarm the managed context. The real reasons are
        // independent of the pause, and either one alone suffices:
        //
        //   1. CompactAgainst proves absence for itself. It snapshots the live slots AND the pending-push
        //      cells, applies the insertion-sequence horizon, and requires TWO consecutive clean passes.
        //      Not one step of that consults _samplingActive or assumes a quiescent map -- it is the same
        //      proof against a map being rewritten as against a parked one.
        //   2. A pass during a pause is *strictly safer* than an active one with respect to the documented
        //      residual race: no sample is taken while paused, and the resume's Start() bumps
        //      NewGeneration(), which invalidates every slot written before the resume -- so even a wrongly
        //      reclaimed entry cannot yield a stale link that survives into the next sampling session.
        //
        // MUST be called with _mtx_wake NOT held.
        void CompactRetiredSpansIfDue(std::chrono::steady_clock::time_point& nextCompaction,
            std::chrono::milliseconds interval) noexcept
        {
            if (std::chrono::steady_clock::now() < nextCompaction)
                return;

            _retiredSpans.CompactAgainst(_traceContexts, _pendingPushes);
            nextCompaction = std::chrono::steady_clock::now() + interval;
        }

        // Surface registry overflow at a production-visible level, edge-triggered: the latch re-arms on the
        // first batch that adds no new drops, so a SUSTAINED episode warns exactly once. A workload
        // hovering right at capacity, alternating dropping and non-dropping batches, can therefore warn
        // more than once per episode -- accepted, and the same shape as the existing truncation latch.
        // What it never does is stay silent for a genuinely new episode.
        //
        // An overflowed insert is a retirement that was DROPPED, which means a stale link can
        // survive -- the one honest asterisk on "provably zero false positives", so it must never be
        // silent. Called from RetireSpans on arbitrary app / finalizer threads; the latch is only written
        // here and a benign race merely repeats or skips one warning.
        void ReportRetiredSpanOverflowIfNeeded() noexcept
        {
            const uint64_t overflows = _retiredSpans.OverflowCount();
            if (overflows == _reportedRetiredSpanOverflowCount.load(std::memory_order_relaxed))
            {
                // No new drops -> re-arm for a fresh episode.
                _loggedRetiredSpanOverflowWarning.store(false, std::memory_order_relaxed);
                return;
            }

            _reportedRetiredSpanOverflowCount.store(overflows, std::memory_order_relaxed);

            if (!_loggedRetiredSpanOverflowWarning.exchange(true, std::memory_order_relaxed))
            {
                LogWarn(L"CP: retired-span registry full; dropped a span retirement. Samples on an idle "
                    L"thread may stay linked to an already-ended span until that thread pushes again. "
                    L"** THIS MAY RESULT IN INCORRECT TRACE CORRELATION ** (total drops so far: ",
                    overflows, L")");
            }
        }
        ...
        void SignalShutdown() noexcept
        {
            ...
            // evaluating its wait predicate and blocking. A lost shutdown signal would delay
            // Shutdown()'s join() by up to one wait period -- CompactionIntervalMs when the worker is
            // paused, or the sampling interval when it is active -- rather than hanging it forever.
            //
            // It used to be "hang forever", because the paused wait had no timeout at all. That changed
            // with CompactRetiredSpansIfDue. Anyone considering reinstating the untimed paused wait must
            // reinstate this hazard too: the ONLY thing standing between a lost signal and a permanently
            // unjoinable worker would again be this store being made under _mtx_wake.
            {
                std::lock_guard<std::mutex> l(_mtx_wake);
        ...
        // interval (or until woken by Stop()/Shutdown()) and capture a sample; while paused, wait up to
        // CompactionIntervalMs (or until woken by Start()/Shutdown()) and run only the retired-span
        // compaction, never a capture. Terminates when _shuttingDown is true. The paused wait is timed
        // rather than indefinite specifically so compaction is not gated on sampling -- see
        // CompactRetiredSpansIfDue.
        void SamplingThreadStart()
        {
            ...
            // Start of the PREVIOUS capture tick, used to measure the REAL tick-to-tick period shipped in
            // BatchStats.actualSamplePeriodMs. NextWaitMs floors the inter-tick wait at intervalMs/2, so an
            // overrunning capture stretches the real period past the configured interval; reporting it lets
            // the managed side credit each sample the time it actually represents. Reset to "unknown" on the
            // first tick of a session and after any tick that threw or was preceded by a pause, since neither
            // yields a meaningful preceding-tick timestamp.
            std::chrono::steady_clock::time_point lastCaptureStart{};
            bool haveLastCaptureStart = false;

            // Next scheduled retired-span compaction. Tracked on its own wall clock, independent of the
            // sampling cadence, so the compaction interval cannot drift with the configured interval.
            //
            // The interval is copied into a local first: CompactionIntervalMs is an in-class constant with
            // no out-of-line definition, and std::chrono::milliseconds' converting constructor takes its
            // argument by const& -- which would ODR-use it and leave the symbol undefined at link time on
            // the Linux (Clang, pre-C++17) build, exactly the trap MinIntervalMs needed a namespace-scope
            // definition for.
            const int64_t compactionIntervalMs = CompactionIntervalMs;
            const auto compactionInterval = std::chrono::milliseconds(compactionIntervalMs);
            auto nextCompaction = std::chrono::steady_clock::now() + compactionInterval;
                ...
                {
                    {
                        ...
                        {
                            // Paused: no sample to take until Start() or Shutdown() wakes us, but the
                            // registry still needs compacting (see CompactRetiredSpansIfDue -- the
                            // send-backoff pause keeps retiring), so wake on the compaction interval
                            // rather than waiting indefinitely. This is NOT the pathological re-poll the
                            // pause originally avoided: that was an always-true predicate re-evaluated
                            // every _intervalMs, which returns instantly and pegs a core. One wake per
                            // CompactionIntervalMs to run one off-suspend pass is not that.
                            _cv_wake.wait_for(l, compactionInterval,
                                [&]() noexcept { return _shuttingDown.load() || _samplingActive.load(); });

                            // Both resets below now run on EVERY timed wake, not once per pause as they
                            // did when this wait was untimed. That is harmless and deliberate: they are
                            // idempotent, and while paused there is no preceding capture or preceding tick
                            // for either to describe, so re-zeroing them cannot lose information.
                            //
                            ...
                            lastCaptureMs = 0;
                            // Likewise the pause breaks the tick-to-tick period chain: the first post-pause
                            // tick has no meaningful preceding-tick timestamp, so report 0 (managed then
                            // credits the configured interval) rather than a period spanning the pause.
                            haveLastCaptureStart = false;
                        }
                    }
                    ...
                    if (!_samplingActive.load())
                    {
                        // Still paused (a timed wake, or a Stop() that raced the wait). Compact anyway --
                        // see CompactRetiredSpansIfDue for why this must not be gated on sampling.
                        CompactRetiredSpansIfDue(nextCompaction, compactionInterval);
                        continue;
                    }
                    ...
                    // Real wall-clock period since the previous tick started == the time this tick's samples
                    // actually represent (see the lastCaptureStart declaration). 0 when there is no preceding
                    // tick to measure against.
                    int64_t actualSamplePeriodMs = 0;
                    if (haveLastCaptureStart)
                    {
                        actualSamplePeriodMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                            captureStart - lastCaptureStart).count();
                    }
                    lastCaptureStart = captureStart;
                    haveLastCaptureStart = true;

                    CaptureAllThreads(actualSamplePeriodMs);
                    lastCaptureMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::steady_clock::now() - captureStart).count();

                    // Reclaim retired-span entries that no live slot references any more. Runs on this
                    // thread, AFTER the capture has resumed the runtime -- never inside the suspend
                    // window, and never on an app thread.
                    //
                    // NOT cheap: a pass acquire-loads all Capacity (65,536) entry State fields
                    // unconditionally, and linearly scans the live-id snapshot for each ELIGIBLE entry --
                    // O(occupied x live), not O(n). At the throughput the registry's own sizing comment
                    // contemplates (~3,200 ended spans/s, so ~32,000 occupied entries) against a few
                    // hundred concurrently-live contexts that is tens of millions of int64 comparisons:
                    // tens of milliseconds, not sub-millisecond. That cost delays the NEXT tick and
                    // never extends a suspend window, which is why it is acceptable here and only here.
                    // See CompactAgainst for the complexity statement.
                    //
                    // Gated on its own wall-clock deadline rather than a tick count: a tick count would
                    // make the real cadence a multiple of the configured sampling interval, silently
                    // coupling this to a tuning knob.
                    //
                    // At the default 10s sampling interval the loop only wakes every 10s, so a pass lands
                    // on the first tick after each deadline. That is a CAPACITY effect (slightly higher
                    // steady-state occupancy), never a correctness one -- an entry that outlives its
                    // deadline is simply reclaimed one tick later. Do NOT "fix" it by shortening the
                    // sampling wait or deriving that wait from CompactionIntervalMs; that reintroduces
                    // precisely the coupling this constant exists to avoid.
                    CompactRetiredSpansIfDue(nextCompaction, compactionInterval);
                }
                catch (...)
                {
                    ...
                    lastCaptureMs = 0;
                    // A truncated tick is not a clean period either; drop the chain so the next successful
                    // tick reports 0 (configured interval) rather than a period spanning the failure.
                    haveLastCaptureStart = false;
        ...
        //
        // actualSamplePeriodMs: the real wall-clock period since the previous tick, 0 when unknown. Defaulted
        // so the CaptureOnceForTesting / CaptureAfterStopRaceForTesting seams (which drive a single pass with
        // no preceding tick) call it unchanged and simply report a 0 period.
        void CaptureAllThreads(int64_t actualSamplePeriodMs = 0)
        {
            ...
            // Honor a pending name-cache invalidation from a CLR unload BEFORE resolving this tick's
            // frames (see InvalidateNameCache). Done here, on the worker thread, so the not-thread-safe
            // _nameCache is only ever mutated by its owning thread; clearing first, then re-resolving this
            ...
            // TOCTOU re-check (check-then-act race with Stop()). SamplingThreadStart tests _samplingActive
            // BEFORE calling this function, but that check and the SuspendMutex acquisition above are not
            // atomic: Stop() can win the gap. Stop() stores _samplingActive=false and THEN takes this same
            // SuspendMutex to free the session buffers (_stackwalk/_capture/... via ReleaseSamplingResources).
            // Because Stop() frees under SuspendMutex, a worker that only reaches the mutex AFTER Stop()
            // released it would otherwise re-allocate the just-freed buffers here, suspend the runtime, and
            // publish one stale batch into the queue Stop() just reset -- defeating Stop()'s memory-reclaim
            // guarantee and handing the managed side a post-stop batch. Re-validating the flag now that we
            // hold the mutex Stop() serializes its free on closes that window: if Stop()'s free has completed
            // we are guaranteed to observe _samplingActive==false (its store happens-before its SuspendMutex
            // release, which happens-before our acquire) and bail before touching anything. The only capture
            // that still publishes is one already past this point when Stop() was called -- the accepted,
            // bounded single-in-flight tick, sampled while the session was genuinely active.
            if (!_samplingActive.load())
            {
                LogTrace(L"CP: sampling stopped before this capture acquired the suspend mutex; skipping tick (Stop won the race)");
                return;
            }
            ...
            {
                // Same silent-data-loss family as the mid-batch/publish drops in EncodeAndPublish: this
                // tick's stacks are never captured, so count it as a dropped tick (reported by the next
                // batch that publishes) and warn once per episode. Edge-triggered on the shared latch that
                // EncodeAndPublish's successful publish resets, so a slow-reader run warns once, not every
                // sampling interval.
                ++_droppedTicksPendingReport;
                if (!_loggedBackpressureDropWarning)
                {
                    LogWarn(L"CP: skipping this tick without suspending -- both sample buffers full (managed reader "
                        L"has not drained). ** THIS WILL RESULT IN LOSS OF PROFILING DATA ** (see the "
                        L"ContinuousProfiling/DroppedTicks supportability metric for the ongoing count)");
                    _loggedBackpressureDropWarning = true;
                }
                return;
            }
            ...
            if (_capture.size() != ThreadCountForReservation)
            {
                // ALL-OR-NOTHING reservation: build into local temporaries and swap() (noexcept, O(1)) into
                // the members only once the whole block succeeds. A bad_alloc partway through is realistic
                // on a memory-capped container and is swallowed by the outer catch(...); mutating members in
                // place could otherwise leave _capture sized but _resolved not, and this guard would then
                // treat that torn state as "done" forever -- risking an out-of-bounds _resolved access or an
                // unreserved FunctionIds vector reallocating (and deadlocking the heap) inside the suspend
                // window. Temporaries keep the members untouched on failure, so the next tick retries clean.
                std::vector<CapturedThread> newCapture(ThreadCountForReservation);
                for (auto& slot : newCapture)
                {
                    slot.FunctionIds.reserve(MaxStackFramesSupported);
                ...
                // Test seam: model a bad_alloc landing PARTWAY through the block -- after the capture
                // temp is fully built but before _resolved's -- to prove the swap-publish below is
                // genuinely all-or-nothing. No-op in production (the hook is empty). See
                // SetReservationFailpointForTesting.
                if (_reservationFailpointForTesting)
                {
                    _reservationFailpointForTesting();
                }
                ...
                // Reserve the pre-suspend thread-ID list once, matching the capture slots. Into a temp too
                // so the whole block stays atomic -- reserve() straight on the member would be a member
                // mutation that a throw here (or on any allocation above) could leave half-applied.
                std::vector<ThreadID> newThreadList;
                newThreadList.reserve(ThreadCountForReservation);

                // Every allocation above succeeded -- publish atomically. These swaps are noexcept.
                _capture.swap(newCapture);
                _resolved.swap(newResolved);
                _threadList.swap(newThreadList);
            }
...

            {
                ...
                //
                // COVERED (was CP-PR3717-review-round-6.md Cluster 1, also round-4 Cluster J / M1 native):
                // the three failure branches below -- an exception during the suspend window must still
                // ResumeRuntime (the try/catch(...) + resume just below), a failed SuspendRuntime must NOT
                // call ResumeRuntime (early return in the FAILED(suspendHr) block), and a failed ResumeRuntime
                // must take the warning path (FAILED(resumeHr) block) -- previously had zero native unit-test
                // coverage because StubCorProfilerInfo4 / RichStubCorProfilerInfo4 refused ICorProfilerInfo10,
                // so this whole branch never executed under test. Now covered via ScriptableSuspendResumeCor-
                // ProfilerInfo (a scriptable ICorProfilerInfo10 stub with return-code/call-count control) plus
                // the _captureWindowFailpointForTesting seam; see ContinuousProfilerTest/
                // ContinuousProfilerCaptureTest.cpp (TEST_CLASS ContinuousProfilerSuspendResumeTest).
                if (_isCoreClr)
                {
                ...
                try
                {
                    // Test seam: model a fault INSIDE the suspend window (see SetCaptureWindowFailpointForTesting).
                    // No-op in production (the hook is empty). Placed inside the try, after a successful
                    // SuspendRuntime, so it exercises the catch(...) -> ResumeRuntime pairing below.
                    if (_captureWindowFailpointForTesting)
                    {
                        _captureWindowFailpointForTesting();
                    }
            ...
            EncodeAndPublish(failedSnapshotCount, batchTimestamp, microsSuspended, overflowCount, actualSamplePeriodMs);
        }
        ...
        // deferredThreadCount: threads live this sweep but deferred to a later round-robin tick because they
        // exceeded the capture-slot capacity (== overflowCount from ProfileAllThreads); shipped so the
        // managed side can detect and correct >100-thread round-robin under-sampling.
        // actualSamplePeriodMs: the real tick-to-tick period this batch represents.
        void EncodeAndPublish(uint32_t failedSnapshotCount, int64_t batchTimestamp, int64_t microsSuspended,
            uint32_t deferredThreadCount, int64_t actualSamplePeriodMs)
        {
            try
            {
                ...
                for (size_t i = 0; i < _capturedCount; ++i)
                {
                    ...
                    if (!writer.WillFit(EstimateSampleBytes(resolved, writer) + TrailerBytes))
                    {
                        // Silent data loss: the remaining captured threads are dropped for this tick. Surface
                        // it at a production-visible level AND count it (truncatedThreadCount in BatchStats
                        // below -> managed supportability metric), matching the OTLP egress side's
                        // drop+Warn+metric discipline. Edge-triggered so a sustained overflow episode warns
                        // once rather than every sampling interval; the per-tick count still flows out in the
                        // stats so the true volume is never lost. _loggedMidBatchTruncationWarning is reset
                        // whenever a tick encodes with nothing truncated (see below), so a new episode re-warns.
                        if (!_loggedMidBatchTruncationWarning)
                        {
                            LogWarn(L"CP: encode buffer full mid-batch; truncating the remaining thread(s) this tick. "
                                L"** THIS WILL RESULT IN LOSS OF PROFILING DATA ** (see the ContinuousProfiling/TruncatedThreads "
                                L"supportability metric for the ongoing count)");
                            _loggedMidBatchTruncationWarning = true;
                        }
                        break;
                    }
                ...
                // truncation above doesn't overcount. deferredThreadCount + actualSamplePeriodMs are the v4
                // additions; actualSamplePeriodMs is clamped into int32 range (period in ms never approaches
                // that ceiling in practice, but a defensive cast keeps the wire field well-defined).
                const int32_t actualSamplePeriodMsField = actualSamplePeriodMs > static_cast<int64_t>((std::numeric_limits<int32_t>::max)())
                    ? (std::numeric_limits<int32_t>::max)()
                    : (actualSamplePeriodMs < 0 ? 0 : static_cast<int32_t>(actualSamplePeriodMs));

                // v5 additions. truncatedThreadCount = threads captured this tick but dropped because the
                // encode buffer filled mid-batch (the loop broke above). droppedTickCount = whole ticks
                // discarded under back-pressure since the last successfully published batch (the two
                // HasFreeSlot/TryPublish drop paths); shipped here so a slow-reader episode still surfaces
                // as a managed supportability metric even though those ticks produced no batch of their own.
                const int32_t truncatedThreadCount = static_cast<int32_t>(_capturedCount - emittedCount);
                const int32_t droppedTickCount = static_cast<int32_t>(_droppedTicksPendingReport);
                writer.WriteBatchStats(microsSuspended, emittedCount, totalFrames,
                    static_cast<int32_t>(failedSnapshotCount),
                    static_cast<int32_t>(deferredThreadCount), actualSamplePeriodMsField,
                    truncatedThreadCount, droppedTickCount);
                writer.WriteEndBatch();

                // A tick that truncated nothing clears the edge-trigger latch, so the next overflow episode
                // warns afresh rather than staying silent for the life of the process.
                if (truncatedThreadCount == 0)
                {
                    _loggedMidBatchTruncationWarning = false;
                }
                ...
                {
                    // Published: the droppedTickCount just shipped is now delivered, so clear the pending
                    // tally and reset the back-pressure warn latch (the reader has caught up).
                    _droppedTicksPendingReport = 0;
                    _loggedBackpressureDropWarning = false;
                }
                else
                {
                    // This whole batch is lost -- count it as a dropped tick (reported by the next batch that
                    // does publish) and warn once per episode, matching the mid-batch-truncation path.
                    ++_droppedTicksPendingReport;
                    if (!_loggedBackpressureDropWarning)
                    {
                        LogWarn(L"CP: encoded batch dropped -- both sample buffers full (managed reader has not drained). "
                            L"** THIS WILL RESULT IN LOSS OF PROFILING DATA ** (see the ContinuousProfiling/DroppedTicks "
                            L"supportability metric for the ongoing count)");
                        _loggedBackpressureDropWarning = true;
                    }
                }
        ...
        // Upper-bound byte estimate for one sample, used by the overflow guard. Frame cost is charged
        // against the writer's CURRENT interning table (via EstimateFrameBytes): a frame already seen this
        // batch costs only its 2-byte back-reference, so shared framework frames -- the common case across
        // 100+ threads -- are not each charged as a fresh full definition. Charging them fresh made the
        // mid-batch truncation fire far earlier than the real 4MB budget required. Still a strict upper
        // bound (see EstimateFrameBytes), so WillFit can never admit a sample that then overflows.
        static size_t EstimateSampleBytes(const ResolvedThread& thread, const SampleBufferWriter& writer) noexcept
        {
            // 1 opcode + name len prefix(2) + 4 int64 fields(32) + isAgentWork byte(1) +
            // frame terminator(2).
            ...
            {
                bytes += writer.EstimateFrameBytes(frame);
            }
            return bytes;
        }
        ...
        uint32_t EnrichCapturedThreads()
        {
            ...
            for (size_t i = 0; i < _capturedCount; ++i)
            {
                ...
                // the name lookup below already treats 0 / an unreadable id as "no name".
            ...
            uint32_t& truncatedStackCount)
        {
            ...
            for (size_t visited = 0; visited < window.visitCount; ++visited)
            {
                ...
                try
                {
                    ...
                    // SuspendRuntime is called unconditionally in the code, gated at RUNTIME by _isCoreClr
                    // (NOT compile-time PAL_STDCPP_COMPAT -- see CaptureAllThreads and Init), so it runs on
                    // CoreCLR on every OS including Windows. On .NET Framework (_isCoreClr false) it is
                    // skipped and DoStackSnapshot's own per-target-thread suspend is the sole freeze, ending
                    // the instant DoStackSnapshot returns below. Either way the trace-context stamp here
                    // relies only on DoStackSnapshot's per-thread suspend, which holds on every platform.
                    // Declared local
                    ...
                    ThreadProfile threadProfile(threadId, _corProfilerInfo, _nameCache, *_stackwalk, _traceContexts, _retiredSpans, threadContext);
                    ...
                    // Thread* getter -- no allocation, no metadata -- so it is safe under suspend. The
                    // HRESULT IS checked: defaulting a failure to 0 would let two different unresolved
                    // threads collide on the same synthetic OsThreadId and cross-contaminate each other's
                    // resolved thread name / managed-thread-id lookup. NextSyntheticThreadId() mints a fresh,
                    // never-repeating id from the top of the DWORD range instead, so a failure is always its
                    // own key.
                    captured.OsThreadId = 0;
                    if (FAILED(_corProfilerInfo->GetThreadInfo(threadId, &captured.OsThreadId)))
                    {
                        captured.OsThreadId = NextSyntheticThreadId();
                    }
        ...
        static HRESULT __stdcall StaticStackFrameCallback(uintptr_t functionId, uintptr_t /* instructionPointer */, uintptr_t /* frameInfo */, uint32_t /* contextSize */, uint8_t[] /* context */, void* clientData)
        {
            try
            {
                ...
                {
                    // Resolve this thread's context, then reject it if its span's transaction has already
                    // ended. The second check is what closes the cross-thread gap: this thread's slot may
                    // still hold a context whose transaction ended on a different thread, and nothing can
                    // clear this slot except this thread itself.
                    //
                    // Still wait-free, lock-free, allocation-free, and clock-free: Contains is a bounded
                    // probe of at most MaxProbes acquire-loads, the same computational shape as the slot
                    // probe TryGet just performed. Nothing here can block a suspended thread.
                    if (threadProfile._traceContexts.TryGet(threadProfile._managedTID, threadProfile._contextOut)
                        && threadProfile._retiredSpans.Contains(threadProfile._contextOut.SpanId))
                    {
                        // Same outcome as a genuine no-context slot: the sample is kept, only the link is
                        // dropped (the managed OtlpProfileBuilder encodes zeros as link index 0).
                        threadProfile._contextOut = TraceContext{};
                    }
        ...
        // Test-only lifecycle hooks (default-inert; only ever set by ContinuousProfilerLifecycleTest).
        // _failWorkerCreationForTesting forces one Start()'s std::thread creation to throw so the failure/
        // catch path can be driven deterministically; _onStartCatchForTesting runs at the top of that catch
        // handler so a probe thread can observe whether the lifecycle lock is still held. See
        // SetFailWorkerCreationForTesting / SetOnStartCatchForTesting.
        std::atomic<bool> _failWorkerCreationForTesting{ false };
        std::function<void()> _onStartCatchForTesting;

        // _reservationFailpointForTesting fires partway through CaptureAllThreads' per-session reservation
        // block (after the _capture temp, before the _resolved temp), so a test can throw there to prove
        // the reservation is all-or-nothing. Empty in production -- one null-function check on the rare
        // first-tick/retune reservation path only. See SetReservationFailpointForTesting.
        std::function<void()> _reservationFailpointForTesting;

        // _captureWindowFailpointForTesting fires INSIDE the stop-the-world window in CaptureAllThreads --
        // after a successful SuspendRuntime, in the same try block as ProfileAllThreads -- so a test can
        // throw there to model a capture that faults while the runtime is suspended, and assert the
        // catch(...) still lets ResumeRuntime run (never leaving the runtime permanently suspended). Empty
        // in production -- one null-function check per tick. See SetCaptureWindowFailpointForTesting.
        std::function<void()> _captureWindowFailpointForTesting;
        ...
        // Span ids whose transaction has ENDED, so the sampler can refuse to link a sample to a dead
        // span. Separate from _traceContexts on purpose: transaction end runs on whatever thread
        // completes (or finalizes) the transaction, which is routinely NOT the thread that pushed the
        // context, and clearing that thread's slot from here would break the single-writer-per-slot
        // invariant _traceContexts depends on. Invalidating the VALUE instead of the SLOT keeps that
        // invariant intact by construction. Read inside the suspend window (wait-free), written from
        // arbitrary app threads and from the GC finalizer thread, compacted on the sampling thread.
        RetiredSpanRegistry _retiredSpans;

        // Per-thread cell in which an app thread publishes the span id it is ABOUT to push, so a
        // compaction pass can count an in-flight push as live. Narrows -- does not close -- the window in
        // which a snapshot cannot see a push that has been decided but not yet published; see
        // PendingPushMap.h and RetiredSpanRegistry::CompactAgainst for exactly what it does and does not
        // prove. Keyed by ThreadID and never tombstoned by a transaction event, which is why it cannot be
        // a field in a _traceContexts slot.
        PendingPushMap _pendingPushes;

        // Edge-triggered latch for the registry-overflow warning, so a sustained overflow episode warns
        // once rather than on every terminating transaction. Mirrors _loggedMidBatchTruncationWarning --
        // but ATOMIC, because unlike that sampling-thread-only latch this one is touched from arbitrary app
        // threads and from the GC finalizer thread (see RetireSpans). Relaxed throughout: the two fields
        // are not updated as a pair, so a race can still repeat or skip one warning, which is accepted and
        // is the only thing at stake. What the atomics buy is that it is a defined outcome rather than a
        // data race.
        std::atomic<bool> _loggedRetiredSpanOverflowWarning{ false };

        // Overflow total already reported, so the warning re-arms for a genuinely new episode.
        std::atomic<uint64_t> _reportedRetiredSpanOverflowCount{ 0 };
        ...
        // Source of synthetic OS thread ids handed out by NextSyntheticThreadId() when GetThreadInfo fails
        // (see ProfileAllThreads). Starts at the top of the DWORD range and counts down: real OS thread ids
        // (Windows tids, Linux pids/tids) stay far below this range in practice, and each call returns a
        // fresh value, so two failures -- in the same tick or different ones -- never collide with each
        // other or with a real id. Touched only by the sampler thread, under suspend.
        DWORD _nextSyntheticThreadId{ 0xFFFFFFFFu };

        // See _nextSyntheticThreadId. Never returns 0 (reserved for "not yet resolved" / unused slots)
        // or wraps back into real-id territory within any realistic process lifetime.
        DWORD NextSyntheticThreadId() noexcept
        {
            return _nextSyntheticThreadId--;
        }
        ...
        // BatchStats opcode (1 + int64 + int32 x7 = 37 bytes, the seven int32s being threadCount, frameCount,
        // skipped, the v4 deferredThreadCount + actualSamplePeriodMs, and the v5 truncatedThreadCount +
        // droppedTickCount) + one EndBatch opcode (1 byte). Reserved in the per-sample WillFit check so a
        // maxed-out batch can't overshoot MaxBufferBytes.
        static constexpr size_t TrailerBytes = 38;
        ...
        // Whole ticks discarded under back-pressure (HasFreeSlot gate in CaptureAllThreads, or a failed
        // TryPublish in EncodeAndPublish) since the last successfully published batch. Shipped in that
        // next batch's v5 BatchStats (droppedTickCount) and then reset, so the managed side surfaces the
        // loss as a supportability metric even though these ticks produce no batch of their own. Only ever
        // touched on the sampling thread (both drop sites and EncodeAndPublish run there), so no atomic.
        uint32_t _droppedTicksPendingReport{ 0 };

        // Edge-trigger latch: set when the mid-batch encode-buffer overflow first warns, cleared when a tick
        // encodes with nothing truncated, so a sustained overflow episode warns once (not every interval)
        // while the per-tick truncatedThreadCount still flows out in BatchStats. Sampling-thread only.
        bool _loggedMidBatchTruncationWarning{ false };

        // Edge-trigger latch for the back-pressure drop paths (HasFreeSlot gate + failed TryPublish): set on
        // the first drop of an episode, cleared when a batch publishes successfully. Sampling-thread only.
        bool _loggedBackpressureDropWarning{ false };
    };
