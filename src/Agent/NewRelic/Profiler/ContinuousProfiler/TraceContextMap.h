...
namespace NewRelic { namespace Profiler { namespace ContinuousProfiler
{
    ...
    public:
        ...
        //
        // Callers: the owning thread itself (SetTraceContext/ResetTraceContext), and
        // ContinuousProfiler::ThreadDestroyed running on the CLR's thread-destruction callback thread --
        // i.e. NOT the owning thread. That cross-thread call still satisfies the invariant above because,
        // per Microsoft's ICorProfilerCallback::ThreadDestroyed documentation, the callback fires only
        // after the thread has already exited, so the owning thread can no longer be concurrently
        // writing this slot when Reset runs on its behalf.
        ...
        // Copy out every span id currently held by a live slot. This is the compaction pass's only view
        // of live state: an entry in RetiredSpanRegistry may be reclaimed only if it is absent from this
        // snapshot, which is what makes reclamation proven rather than guessed.
        //
        // READ-ONLY, and that is load-bearing: this runs on the sampling thread against slots owned by
        // arbitrary app threads, so writing anything here would break the single-writer-per-slot
        // invariant that makes every write in this file race-free. It uses the same seqlock read pattern
        // TryGet already uses -- a cross-thread read of every other thread's slot is already how the
        // sampler works, so this introduces no new synchronization risk.
        //
        // Unlike TryGet this is NOT called under suspend and NOT on a hot path, so it is allowed to
        // retry a torn slot a bounded number of times rather than immediately giving up: a snapshot that
        // silently omitted a live span would let compaction reclaim an entry a slot still references.
        // If any slot still cannot be read cleanly, or the caller's buffer is too small, this returns
        // false and the caller MUST reclaim nothing this pass.
        //
        // Deliberately ignores the generation stamp. A slot from a previous session can never produce a
        // link (TryGet rejects it), so reporting it is unnecessary -- but over-reporting only postpones a
        // reclamation, while under-reporting risks a false positive, so bias to over-report. Note the
        // postponement is INDEFINITE, not one pass, for a slot whose owning thread never pushes again:
        // nothing rewrites that slot, so its span reads live forever and its registry entry can never be
        // reclaimed. Bounded and benign -- at most one stuck entry per map slot (SlotCount), against a
        // registry an order of magnitude larger -- but it is a permanent hold, not a delay.
        bool SnapshotLiveSpanIds(int64_t* out, size_t capacity, size_t& count) const noexcept
        {
            count = 0;
            if (out == nullptr)
            {
            ...
            for (size_t i = 0; i < SlotCount; ++i)
            {
                ...
                int64_t span = 0;
                bool read = false;
                for (size_t attempt = 0; attempt < SnapshotReadAttempts; ++attempt)
                {
                    const uint32_t seqBefore = slot.Seq.load(std::memory_order_acquire);
                    if ((seqBefore & 1u) != 0u)
                    {
                        continue; // write in progress -> retry (we are not under suspend here).
                    }

                    const int64_t candidate = slot.Span.load(std::memory_order_relaxed);
                    std::atomic_thread_fence(std::memory_order_acquire);

                    if (slot.Seq.load(std::memory_order_acquire) == seqBefore)
                    {
                        span = candidate;
                        read = true;
                        break;
                ...
                if (!read)
                {
                    return false; // could not observe this slot cleanly -> caller must not reclaim.
                }

                if (span == 0)
                {
                    continue; // reset or never-set context -> references no span.
                }
    ...
    private:
        ...
        // Bounded retries for a torn slot read in SnapshotLiveSpanIds. Safe because that method, unlike
        // TryGet, never runs inside the suspend window -- so a brief retry cannot deadlock against a
        // frozen writer. A slot that stays torn across all attempts fails the whole snapshot rather than
        // being silently omitted.
        //
        // Like every in-class constant here it has no out-of-line definition, so it must never be
        // ODR-used: never bind it to a reference, never take its address, never pass it to anything
        // taking a reference (Assert::AreEqual included) -- copy it to a local first. That constraint is
        // about the USAGE, not the keyword; static const and static constexpr behave identically here.
        static constexpr size_t SnapshotReadAttempts = 8;
