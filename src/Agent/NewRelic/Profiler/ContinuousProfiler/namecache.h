...
// Continuous profiler's own name cache. Deliberately NOT shared with
// NewRelic::Profiler::ThreadProfiler::NameCache: the thread profiler defers all name
// resolution to the end of a bounded profiling window and needs its cache to stay
// permanent (unbounded) for that to be correct, while CP runs indefinitely and needs a
// bounded cache to avoid unbounded growth. Bounding the shared class once broke the
// thread profiler's deferred-resolution invariant -- keeping the two independent avoids
// that class of bug recurring.
namespace NewRelic {
    namespace Profiler {
        namespace ContinuousProfiler
        {
            ...
            //
            // NOT thread safe: the continuous profiler's sampling thread owns its instance and touches it
            // from one thread at a time. NOT shared with the thread profiler -- see namecache.h header
            // comment for why the two caches are deliberately separate classes.
            ...
            class NameCache
            {
                ...
                // while keeping the worst case bounded -- CP runs for the life of the process, so both maps
