...
function Copy-ExtensionsOther {
    ...
    if ($Type -like "Framework"){
        Copy-Item -Path "$RootDirectory\src\_build\AnyCPU-$Configuration\NewRelic.Core\net35\NewRelic.Core.dll" -Destination "$Destination\extensions" -Force 
        Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Agent\Parsing\bin\$Configuration\net35\NewRelic.Parsing.dll" -Destination "$Destination\extensions" -Force 
    }

    if ($Type -like "Core"){
        Copy-Item -Path "$RootDirectory\src\_build\AnyCPU-$Configuration\NewRelic.Core\netstandard2.0\NewRelic.Core.dll" -Destination "$Destination\extensions" -Force 
...
function Copy-AgentRoot {
    ...
    if ($Architecture -like "x64" ) {
        Copy-Item -Path "$RootDirectory\src\Agent\ProfilerBuildsForDevMachines\Windows\x64\NewRelic.Profiler.dll" -Destination "$Destination" -Force 
    }
    else {
        Copy-Item -Path "$RootDirectory\src\Agent\ProfilerBuildsForDevMachines\Windows\x86\NewRelic.Profiler.dll" -Destination "$Destination" -Force 
    }
}
