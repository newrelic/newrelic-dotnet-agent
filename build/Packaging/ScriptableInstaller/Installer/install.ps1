...
Function CheckDotNetVersions {
	try {
		...
		$has45Plus = $False | Out-Null

		foreach($result in $results) {
			$versionParts = ([System.String]$result.Version).Split('.')
			if ([System.Decimal]($versionParts[0] + "." + $versionParts[1]) -ge 4.5) {
				$has45Plus = $True
				break;
			}
		}
		...
		if (-Not($has45Plus)) {
			LogEntry "ERROR: This version of the agent requires .NET 4.5 or higher to be installed." $True
			return $False
		}
	}
	catch {
