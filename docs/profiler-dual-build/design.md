...
|---|---|---|---|
| `.deb` / `.rpm` count | 2 (one per arch) -- same as today | 4 (x libc x arch) | 2 (x arch) |
...
Backwards-compat: **hardcoding `CORECLR_PROFILER_PATH=$NRHOME/libNewRelicProfiler.so`** in the environment (Dockerfile `ENV`, systemd unit, k8s manifest) rather than sourcing `setenv.sh` is the pattern the project's own published Dockerfile examples and install docs lead users to (see "What the published guidance leads to" below). Treat it as the dominant Linux install pattern. Removing the flat path without a compat shim would break the majority of Linux installs.
...
- **P2.4 (#3593, merged 2026-05-16 as `d8b3ce811`):** arm64 CI consolidated onto `docker compose run build_arm64`. Required because the dual-build adds two more compose services and we need a uniform model. [done] Done.
- **P2.5 (follow-up, merged):** arm64 profiler build switched to native `ubuntu-24.04-arm` runner. Improves arm64 build performance and removes the QEMU dependency. [done] Done.
...
The naive Phase 2 introduces a **major breaking change** for **the dominant Linux install pattern**: hardcoded `CORECLR_PROFILER_PATH=$NRHOME/libNewRelicProfiler.so`. The project's own published examples and install docs lead users to hardcode this env var rather than source `setenv.sh`, so treat it as the prevailing pattern for `.deb`/`.rpm`/tarball installs. **The compat-symlink mitigations below are therefore load-bearing, not optional belt-and-suspenders.** They are the mechanism by which Phase 2 ships non-breaking for the majority of Linux installs; without them, Phase 2 is a major breaking release.

### What the published guidance leads to (verified)

The install-pattern claims here are grounded in concrete, publicly verifiable evidence:
...
What the published guidance confirms:
- Hardcoding the flat path is the pattern every published example and install-doc page above leads users to; sourcing `setenv.sh` is the alternative tab, not the lead. The design must treat hardcoded `CORECLR_PROFILER_PATH` as the dominant install pattern.
...
4. ~~**Usage data** on how many active installs hardcode `CORECLR_PROFILER_PATH=...flat`.~~ **Resolved: the published examples and install docs lead users to the flat path, so treat it as the dominant install pattern.** The compat-symlink mitigations are load-bearing for non-breaking shipping; this is no longer an open item.
