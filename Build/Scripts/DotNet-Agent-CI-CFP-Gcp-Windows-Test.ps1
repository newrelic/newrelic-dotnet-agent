...
function GetConnectionStringFromLog() {
    ...
    Write-Host "Connect Info:"
    Write-Host "$connectInfo"
    return $connectInfo
}
...
function ValidateConnectData($connectData) {
    ...
    $id = $data.utilization.vendors.gcp.id
    $machineType = $data.utilization.vendors.gcp.machinetype
    $name = $data.utilization.vendors.gcp.name
    $zone = $data.utilization.vendors.gcp.zone

    Write-Host "Vendor Data:"
    Write-Host "id: $id machineType: $machineType name: $name zone: $zone"

    if(($id -ne $null) -and ($machinetype -ne $null) -and ($name -ne $null) -and ($zone -ne $null))
    {
        return $true
    }
    return $false
