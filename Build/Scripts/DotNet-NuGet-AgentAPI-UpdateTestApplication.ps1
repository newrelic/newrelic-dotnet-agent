...
$packageId = "NewRelic.Agent.Api"
$packagesConfigPath = "$env:WORKSPACE\DotNet-Functional-NuGet-AgentAPI\DotNet-Functional-NuGet-AgentAPI\packages.config"
$solutionDir = "DotNet-Functional-NuGet-AgentAPI/"
$solutionPath = "DotNet-Functional-NuGet-AgentAPI\DotNet-Functional-NuGet-AgentAPI.sln"


[Xml]$packages = Get-Content $packagesConfigPath
$oldVersion = $packages.packages.SelectSingleNode("package[@id='$packageId']").version

& C:\NuGet.exe restore $solutionPath -NoCache -Source "http://win-nuget-repository.pdx.vm.datanerd.us:81/NuGet/Default;https://www.nuget.org/api/v2"
& C:\NuGet.exe update $solutionPath -NonInteractive -Id $packageId -Source "http://win-nuget-repository.pdx.vm.datanerd.us:81/NuGet/Default;https://www.nuget.org/api/v2"
...
Invoke-RestMethod -Uri "$($env:BUILD_URL)submitDescription?description=$newVersion" -Method POST -Headers @{'Authorization'=$authorization}
