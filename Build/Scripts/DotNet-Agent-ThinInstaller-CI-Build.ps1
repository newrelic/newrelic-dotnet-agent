...
#Invoke-RestMethod -Uri "$($env:BUILD_URL)submitDescription?description=$agentVersion - $env:GIT_BRANCH - $Commit" -Method POST -Headers @{'Authorization'=$authorization}
#
#if (!$env:sha1 -and $env:BUILD_CAUSE_UPSTREAMTRIGGER)
#{
#    Invoke-RestMethod -Uri "$($env:UPSTREAM_BUILD_URL)submitDescription?description=$agentVersion - $env:GIT_BRANCH - $Commit" -Method POST -Headers @{'Authorization'=$authorization}
#}
