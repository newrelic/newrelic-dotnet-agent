...
function ValidateConnectData($connectData) {
    ...
    if(($data.utilization.vendors.aws.availabilityzone -ne $null) -and ($data.utilization.vendors.aws.instanceid -ne $null) -and ($data.utilization.vendors.aws.instancetype -ne $null))
    {
        return $true
    }
    return $false
