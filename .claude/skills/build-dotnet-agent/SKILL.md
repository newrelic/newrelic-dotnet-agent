...
- **Profiler** (`Profiler.sln`, `src/Agent/NewRelic/Profiler/`, only when changing the profiler): build with `build.ps1 -Platform x64 -Configuration Release`. **Release is the default (`build.ps1` sets it) and the ONLY config to ship or to perf-measure** — a Debug native profiler runs ~20–28% slower in the per-thread stack-walk / suspend hot path (measured 2026-07-16: Debug ~1021µs vs Release ~800µs median suspend, same ~28 threads). Never perf-measure or deploy a Debug profiler; use Debug only when actively debugging the profiler itself. (The `dotnet build FullAgent.sln` **Debug** guidance above applies to the MANAGED build only — it does NOT govern the native profiler's config.) Prefer `build.ps1`; if PowerShell execution policy blocks it and `-ExecutionPolicy Bypass` is denied, run the identical command it issues — from Git Bash prefix `MSYS_NO_PATHCONV=1` and quote the path so the `/restore` `/p:` switches aren't mangled: `MSYS_NO_PATHCONV=1 "<vswhere-resolved MSBuild.exe>" /restore /p:Platform=x64 /p:Configuration=Release "src\Agent\NewRelic\Profiler\NewRelic.Profiler.sln"`. ⚠ `FullAgent.sln` overwrites the home profiler with the NuGet one — re-copy the freshly built `src/Agent/_profilerBuild/x64-Release/NewRelic.Profiler.dll` into the home AFTER a FullAgent build (hash-verify). Linux build needs Docker Desktop.
