...
3. **`AzureFunction` namespace: Core Tools must be >= 4.13.0.** Symptom: `Timed out waiting for Azure function host to become ready`, and the captured `func` stdout carries `Error configuring services in an external startup class ... Could not load file or assembly 'Microsoft.Extensions.Options, Version=10.0.0.0'`. Cause: the isolated `AzureFunctionApplication` extension bundle resolves `Microsoft.Extensions.* 10.0.x` (`Worker.Extensions.Storage.Queues` -> `WebJobs.Extensions.Storage.Queues 5.3.8` -> `Microsoft.Extensions.Hosting >= 10.0.3`) and does not ship those assemblies; the host supplies them. Core Tools retargeted to .NET 10 in 4.13.0 (2026-08-10) -- older hosts are `net8.0` and ship 8.0.0.0. Verify with `func.runtimeconfig.json` (`"tfm": "net10.0"`); winget lags the GitHub release. Select a specific build with `azure_func_exe_path=<...>\func.exe` (CI sets it to the choco path). In-proc classes are unaffected: `AzureFunctionInProcApplication` references the WebJobs extensions directly and builds no bundle.
4. Fixture env vars set? 5. Ports free (localhost)? 6. Agent log: grep `NR-ERROR`/`NR-FATAL`, distilled. 7. App stdout/stderr.
