...
namespace NewRelic.Core.NewRelic.Cache
{
	/// <summary>
	...
	public class SimpleCache<TKey, TValue> : IDisposable where TValue : class
	{
		...
		public SimpleCache(uint capacity)
		{
			Capacity = capacity;
		...
		[MethodImpl(MethodImplOptions.Synchronized)]
		private void SetCapacity(uint newCapacity)
		{
			if (newCapacity < 1)
			{
