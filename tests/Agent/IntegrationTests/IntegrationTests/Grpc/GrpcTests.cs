    ...
    where TFixture : ConsoleDynamicMethodFixture
{
    ...
    public void Metrics_ExternalCallIsCaptured()
    {
        ...
        var expectedMetrics = new List<Assertions.ExpectedMetric>
        {
            ...
            new() { metricName = @"External/localhost/all", CallCountAllHarvests = 1 },
            new() { metricName = @"External/localhost/gRPC/greet.Greeter/SayHello", CallCountAllHarvests = 1 },
            new() { metricName = @"External/localhost/gRPC/greet.Greeter/SayHello", metricScope = "OtherTransaction/Custom/MultiFunctionApplicationHelpers.NetStandardLibraries.Grpc.GrpcExerciser/MakeGrpcClientCall", CallCountAllHarvests = 1 },
        };
    ...
    public void SpanEvents_HaveGrpcAttributes()
    {
        ...
        // ports can vary
        var expectedInstrinsicAttributes = new List<string>
        {
            "server.port"
        };

        var expectedInstrinsicAttributesAndValues = new Dictionary<string, object>
        {
            { "category", "http" },
            { "component", "grpc" },
        ...
        var expectedAgentAttributesAndValues = new Dictionary<string, object>
        {
        ...
        NrAssert.Multiple(
            () => Assertions.SpanEventHasAttributes(expectedInstrinsicAttributesAndValues, SpanEventAttributeType.Intrinsic, externalSpanEvent),
            () => Assertions.SpanEventHasAttributes(expectedAgentAttributesAndValues, SpanEventAttributeType.Agent, externalSpanEvent),
