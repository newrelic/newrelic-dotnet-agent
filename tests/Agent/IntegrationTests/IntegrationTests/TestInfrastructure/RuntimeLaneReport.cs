...
public class RuntimeLaneReport
{
    ...
    public void WriteReport()
    {
        ...
        {
            var isExempt = RuntimeTraitPolicy.ExemptClasses.Contains(type.FullName);
            var lane = isExempt ? RuntimeLane.Core : resolver.Resolve(type);
            var requirement = isExempt ? PlatformTrait.PortableLabel : PlatformTrait.ToRequirementLabel(lane);

            if (requirement == PlatformTrait.UnknownLabel)
            {
                unknown++;
            }

            var fixtureType = RuntimeLaneResolver.GetFixtureType(type);
            builder.Append(type.FullName).Append('\t').Append(requirement).Append('\t').Append(lane).Append('\t')
