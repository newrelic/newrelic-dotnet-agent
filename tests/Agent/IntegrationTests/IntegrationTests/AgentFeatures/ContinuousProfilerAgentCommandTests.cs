...
/// <summary>
/// Exercises the start_continuous_profiling/stop_continuous_profiling agent commands end to end through
...
/// <c>NEW_RELIC_PROFILING_ENABLED</c>) so a session starting is proof the command -- not local
...
public class ContinuousProfilerAgentCommandTests : NewRelicIntegrationTest<AspNetCoreWebApiWithCollectorFixture>
{
    ...
    public ContinuousProfilerAgentCommandTests(AspNetCoreWebApiWithCollectorFixture fixture, ITestOutputHelper output) : base(fixture)
    {
        ...
        _fixture.AddActions(
            setupConfiguration: () =>
            {
                ...
                // Deliberately no NEW_RELIC_PROFILING_ENABLED -- CP must start from the command alone.
            },
            exerciseApplication: () =>
            {
                _fixture.Get();
    ...
    public void StartAndStopCommandsControlContinuousProfilingSession()
    {
        ...
        NrAssert.Multiple(
            () => Assert.True(sessionStarted, "start_continuous_profiling did not start a continuous profiling session."),
            () => Assert.True(sessionStopped, "stop_continuous_profiling did not stop the continuous profiling session."),
