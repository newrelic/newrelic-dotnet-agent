...
namespace NewRelic.Agent.IntegrationTestHelpers
{
	public static class ProcessExtensions
	{
		...
		[NotNull] private static IEnumerable<ManagementObject> GetProcessesRemote([NotNull] String remoteMachineName, UInt32 processId)
		{
			var whereClause = String.Format("ProcessId={0}", processId);
			return GetProcessesRemote(remoteMachineName, whereClause);
		}

		[NotNull] private static IEnumerable<ManagementObject> GetChildProcessesRemote([NotNull] String remoteMachineName, UInt32 parentProcessId)
		{
			var whereClause = String.Format(@"ParentProcessId={0}", parentProcessId);
			return GetProcessesRemote(remoteMachineName, whereClause);
		}

		[NotNull] private static IEnumerable<ManagementObject> GetProcessesRemote([NotNull] String remoteMachineName, [NotNull] String whereClause)
		{
