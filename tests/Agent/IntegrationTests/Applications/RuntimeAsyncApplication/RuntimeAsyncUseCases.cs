...
public class RuntimeAsyncUseCases
{
    ...
    // pool. Measured locally at roughly one dropped segment per twenty nested calls, so the
    // count has to be high enough that a regression cannot slip through on a lucky run.
