...
<configuration xmlns=""urn:newrelic-config"">
  ...
  <applicationPools>
    ...
    {
        _localConfig.highSecurity.enabled = true;
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_ENABLED")).Returns("true");
    ...
    [Test]
    public void ContinuousProfilingEnabled_xml_sets_local_value_below_server_and_env()
    {
        _localConfig.profiling.enabled = true;
    ...
    [Test]
    public void ContinuousProfilingEnabled_env_overrides_xml()
    {
        _localConfig.profiling.enabled = true;
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_ENABLED")).Returns("false");
    ...
    // Precedence: env var > newrelic.config's <profiling enabled="..."/> > default (false). Server-side
    // config (agent_config profiling.enabled) sits above all of this -- see the server/HSM tests above.
    ...
    {
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_ENABLED")).Returns(envValue);

        var defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);
        return defaultConfig.ContinuousProfilingEnabled;
    }

    // profiling.delay / profiling.duration / profiling.include: local/env only, no server-side config
    // surface (per the cross-agent Profiling spec, server-side config is only supported for `enabled`).

    [Test]
    public void ContinuousProfilingDelayMs_defaults_zero()
    {
        Assert.That(_defaultConfig.ContinuousProfilingDelayMs, Is.EqualTo(0));
    }

    [Test]
    public void ContinuousProfilingDelayMs_xml_sets_local_value()
    {
        _localConfig.profiling.delay = 1500;

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingDelayMs, Is.EqualTo(1500));
    }

    [Test]
    public void ContinuousProfilingDelayMs_env_overrides_xml()
    {
        _localConfig.profiling.delay = 1500;
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_DELAY")).Returns("2500");

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingDelayMs, Is.EqualTo(2500));
    }

    [Test]
    public void ContinuousProfilingDurationMs_defaults_zero()
    {
        Assert.That(_defaultConfig.ContinuousProfilingDurationMs, Is.EqualTo(0));
    }

    [Test]
    public void ContinuousProfilingDurationMs_xml_sets_local_value()
    {
        _localConfig.profiling.duration = 60000;

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingDurationMs, Is.EqualTo(60000));
    }

    [Test]
    public void ContinuousProfilingDurationMs_env_overrides_xml()
    {
        _localConfig.profiling.duration = 60000;
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_DURATION")).Returns("120000");

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingDurationMs, Is.EqualTo(120000));
    }

    [Test]
    public void ContinuousProfilingInclude_defaults_all()
    {
        Assert.That(_defaultConfig.ContinuousProfilingInclude, Is.EqualTo(new[] { "all" }));
    }

    [Test]
    public void ContinuousProfilingInclude_xml_sets_local_value()
    {
        _localConfig.profiling.include = "cpu";

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingInclude, Is.EqualTo(new[] { "cpu" }));
    }

    [Test]
    public void ContinuousProfilingInclude_env_overrides_xml_and_splits_comma_list()
    {
        _localConfig.profiling.include = "cpu";
        Mock.Arrange(() => _environment.GetEnvironmentVariableFromList("NEW_RELIC_PROFILING_INCLUDE")).Returns("CPU, Heap");

        _defaultConfig = new TestableDefaultConfiguration(_environment, _localConfig, _serverConfig, _runTimeConfig, _bootstrapConfiguration, _processStatic, _httpRuntimeStatic, _configurationManagerStatic, _dnsStatic);

        Assert.That(_defaultConfig.ContinuousProfilingInclude, Is.EqualTo(new[] { "cpu", "heap" }));
    }
    ...
    // ContinuousProfilingEnabled; start_continuous_profiling/stop_continuous_profiling agent commands mutate
