...
namespace NewRelic.Agent.Core.Logging.Tests
{
  ...
  public class OpenTelemetrySDKLoggerTests
  {
    ...
    #region Integration Tests for Code Coverage

    [Test]
    public void OnEventWritten_IntegrationTest_LogsOpenTelemetryEvents()
    {
      ...
      Mock.Arrange(() => ExtensionsLog.LogMessage(Arg.IsAny<LogLevel>(), Arg.IsAny<string>(), Arg.IsAny<object[]>()))
          .DoInstead<LogLevel, string, object[]>((level, message, args) =>
          {
      ...
      // Create a real EventSource that will trigger OnEventWritten
      using var testEventSource = new TestOpenTelemetryEventSource();
      
      // Manually trigger OnEventSourceCreated to register the event source
      _logger.TriggerOnEventSourceCreated(testEventSource);
      
      // Write test events that should be processed
      testEventSource.WriteInformationalEvent("Test informational message");
      testEventSource.WriteVerboseEvent("Test verbose message");
      testEventSource.WriteWarningEvent("Test warning message");

      // Allow some time for event processing
      System.Threading.Thread.Sleep(100);

      // Verify that events were logged (this tests the actual OnEventWritten method)
      Assert.That(loggedMessages.Count, Is.GreaterThan(0), "Should have logged some OpenTelemetry events");
      
      // Verify the EventLevel.Informational -> LogLevel.Debug mapping works in practice
      if (loggedLevels.Contains(LogLevel.Debug))
      {
        var debugIndex = loggedLevels.IndexOf(LogLevel.Debug);
        Assert.That(loggedMessages[debugIndex], Does.Contain("OpenTelemetrySDK"), 
          "Debug level message should be from OpenTelemetry and contain proper format");
      }
    }

    [Test]
    public void OnEventWritten_IntegrationTest_IgnoresNonOpenTelemetryEvents()
    {
      // Setup logging capture
      Mock.Arrange(() => _serilogLogger.IsEnabled(LogEventLevel.Debug)).Returns(true);
      var loggedMessages = new List<string>();
      ...
      // Create a non-OpenTelemetry EventSource
      using var nonOtelEventSource = new TestNonOpenTelemetryEventSource();
      
      // This should NOT trigger any logging
      _logger.TriggerOnEventSourceCreated(nonOtelEventSource);
      nonOtelEventSource.WriteTestEvent("This should be ignored");

      // Allow some time for processing
      System.Threading.Thread.Sleep(50);

      // Verify no OpenTelemetry messages were logged
      var otelMessages = loggedMessages.Where(m => m.Contains("OpenTelemetrySDK")).ToList();
      Assert.That(otelMessages.Count, Is.EqualTo(0), 
        "Should not log any OpenTelemetrySDK messages for non-OpenTelemetry EventSources");
    }

    [Test]
    public void OnEventWritten_IntegrationTest_HandlesMessageFormatting()
    {
      // Setup logging capture  
      Mock.Arrange(() => _serilogLogger.IsEnabled(LogEventLevel.Debug)).Returns(true);
      var loggedMessages = new List<string>();
      ...
      // Write an event with parameters that need formatting
      testEventSource.WriteParameterizedEvent("Hello", 42);

      System.Threading.Thread.Sleep(100);

      // Verify message formatting worked
      var formattedMessage = loggedMessages.FirstOrDefault(m => m.Contains("Hello") && m.Contains("42"));
      Assert.That(formattedMessage, Is.Not.Null, "Should have formatted the parameterized message");
      Assert.That(formattedMessage, Does.Contain("OpenTelemetrySDK"), "Should contain OpenTelemetry prefix");
    }
    ...
    // - OnEventWritten mapping logic (tested through helper classes AND integration tests)
    ...
    // EventSource classes for integration testing that actually write events
    [EventSource(Name = "OpenTelemetry-IntegrationTest")]
    private class TestOpenTelemetryEventSource : EventSource
    {
      public static TestOpenTelemetryEventSource Instance = new TestOpenTelemetryEventSource();
      ...
      public void WriteVerboseEvent(string message)
      {
        WriteEvent(2, message);
      }

      [Event(3, Level = EventLevel.Warning)]
      public void WriteWarningEvent(string message)
      {
        WriteEvent(3, message);
      }

      [Event(4, Level = EventLevel.Informational)]
      public void WriteParameterizedEvent(string param1, int param2)
      {
        WriteEvent(4, param1, param2);
      }
    }

    [EventSource(Name = "NonOpenTelemetry-Test")]
    private class TestNonOpenTelemetryEventSource : EventSource
    {
      [Event(1, Level = EventLevel.Informational)]
      public void WriteTestEvent(string message)
      {
        WriteEvent(1, message);
      }
    }
