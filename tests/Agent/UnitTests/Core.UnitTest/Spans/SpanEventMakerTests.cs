...
public class SpanEventMakerTests
{
    ...
    #region Span Links Tests

    [Test]
    public void GetSpanEvents_IncludesSpanLinks_WhenSegmentHasLinks()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());

        // Add span links to the segment
        segment.TryAddLinkToSpan("linked-trace-id-1", "linked-span-id-1", new Dictionary<string, object>
        {
            { "link.attr1", "value1" },
            { "link.attr2", 123 }
        });
        segment.TryAddLinkToSpan("linked-trace-id-2", "linked-span-id-2", null);
        ...
        // ACT
        var spanEvents = _spanEventMaker.GetSpanEvents(immutableTransaction, TransactionName, transactionAttribs).ToList();
        var spanEvent = spanEvents[1]; // First segment (skip root span)

        // ASSERT
        Assert.Multiple(() =>
        {
            Assert.That(spanEvent.Span.Links, Is.Not.Null);
            Assert.That(spanEvent.Span.Links.Count, Is.EqualTo(2));
        });
    }

    [Test]
    public void GetSpanEvents_SpanLinksHaveCorrectAttributes()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());

        segment.TryAddLinkToSpan("linked-trace-abc", "linked-span-xyz", new Dictionary<string, object>
        {
        ...
        var spanEvents = _spanEventMaker.GetSpanEvents(immutableTransaction, TransactionName, transactionAttribs).ToList();
        var spanEvent = spanEvents[1];
        var spanLink = spanEvent.Span.Links[0];
        var linkIntrinsics = spanLink.IntrinsicAttributes();
        var linkUserAttribs = spanLink.UserAttributes();
    ...
    [Test]
    public void GetSpanEvents_NoSpanLinks_WhenSegmentHasNoLinks()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());
        var segments = new List<Segment>() { segment };
        ...
        // ASSERT
        Assert.That(spanEvent.Span.Links.Count, Is.EqualTo(0));
    }

    #endregion

    #region Span Events Tests

    [Test]
    public void GetSpanEvents_IncludesSpanEvents_WhenSegmentHasEvents()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());

        var timestamp1 = new DateTime(2024, 1, 1, 12, 0, 0, DateTimeKind.Utc);
        var timestamp2 = new DateTime(2024, 1, 1, 12, 0, 1, DateTimeKind.Utc);

        segment.TryAddEventToSpan("event-1", timestamp1, new Dictionary<string, object>
        {
            { "event.attr1", "value1" },
            { "event.attr2", 456 }
        });
        segment.TryAddEventToSpan("event-2", timestamp2, null);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(spanEvent.Span.Events, Is.Not.Null);
            Assert.That(spanEvent.Span.Events.Count, Is.EqualTo(2));
        });
    }

    [Test]
    public void GetSpanEvents_SpanEventsHaveCorrectAttributes()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());

        segment.TryAddEventToSpan("test-event", DateTime.UtcNow, new Dictionary<string, object>
        {
            { "custom.event.attr", "event-value" }
        });
        ...
        var spanEvents = _spanEventMaker.GetSpanEvents(immutableTransaction, TransactionName, transactionAttribs).ToList();
        var spanEvent = spanEvents[1];
        var eventWireModel = spanEvent.Span.Events[0];
        var eventIntrinsics = eventWireModel.IntrinsicAttributes();
        var eventUserAttribs = eventWireModel.UserAttributes();
    ...
    [Test]
    public void GetSpanEvents_NoSpanEvents_WhenSegmentHasNoEvents()
    {
        // ARRANGE
        var segment = _baseGenericSegment.CreateSimilar(TimeSpan.FromMilliseconds(1), TimeSpan.FromMilliseconds(5), new List<KeyValuePair<string, object>>());
        var segments = new List<Segment>() { segment };
        ...
        // ASSERT
        Assert.That(spanEvent.Span.Events.Count, Is.EqualTo(0));
    }
