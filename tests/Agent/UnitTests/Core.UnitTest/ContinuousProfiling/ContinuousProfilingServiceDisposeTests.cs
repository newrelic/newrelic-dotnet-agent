...
public class ContinuousProfilingServiceDisposeTests
{
    ...
    private sealed class BlockingNativeProfiler : INativeContinuousProfiler
    {
        ...
        public int Start(int intervalMs) => 0;
        ...
        public int SetTraceContext(long traceIdHigh, long traceIdLow, long spanId) => 0;

        public int ResetTraceContext() => 0;

        public int RetireSpans(long[] spanIds, int count) => 0;

        public IntPtr GetPendingPushCell() => IntPtr.Zero;

        public int SetAgentWork() => 0;

        public int ResetAgentWork() => 0;
    }
    ...
    [Test]
    public void A_command_start_racing_a_config_retunes_lock_drop_ends_at_the_command_interval_not_the_configs()
    {
        // Regression test for the interval-retune race. A config retune holds _lifecycleLock across
        // `StopLocked(); StartLocked(configInterval);`, and StopLocked transiently DROPS the lock for its
        // bounded drain wait. A command-driven start (StartFromCommand) that acquires the lock in that window
        // observes a stale _isActive == true -- the retune's stop has not reached its finally yet. Before the
        // fix it claimed command ownership of cpu but treated the start as "already running -- idempotent
        // no-op", so the retune's paired StartLocked restarted at the CONFIG interval and the command's
        // requested interval was silently dropped (and, ownership now claimed, config could never retune it
        // back). The fix waits the in-progress stop out, then -- finding the session restarted at the config
        // interval -- retunes to the command's interval so the command wins, as command ownership requires.
        //
        // Real (non-mock) blocking source, same reason as the sibling tests above: an in-flight drain held
        // inside a mocked ReadBatch's DoInstead would serialize the retune's own pre-wait mock calls behind
        // it and collapse the dropped-lock window this test needs.
        var drainReachedReadBatch = new ManualResetEventSlim(false);
        var releaseReadBatch = new ManualResetEventSlim(false);
        var blockingSource = new BlockingSampleSource(drainReachedReadBatch, releaseReadBatch);
        var intervalMs = 10000;
        ...
        // Capture the interval of every native start in order: the LAST one is the interval the session ends
        // up running at, which is what this race is about.
        var startIntervals = new List<int>();
        Mock.Arrange(() => _native.Start(Arg.IsAny<int>())).DoInstead((int ms) => { lock (startIntervals) startIntervals.Add(ms); });
        var nativeStopCount = 0;
        Mock.Arrange(() => _native.Stop()).DoInstead(() => Interlocked.Increment(ref nativeStopCount));
        ...
        intervalMs = 20000; // an interval change is what makes ApplyConfigChange a StopLocked-then-StartLocked retune
        // Dedicated threads rather than Task.Run, same reason as the sibling tests: a queued thread-pool item
        // may not start executing within the positioning window on a constrained runner, skipping the
        // interleaving under test entirely and passing vacuously. A raw thread starts immediately.
        var retune = new Thread(() => service.ApplyConfigChange()) { IsBackground = true };
        retune.Start();
        Assert.That(retuneReachedStopLocked.Wait(TimeSpan.FromSeconds(5)), Is.True, "the retune must reach StopLocked");

        // The command start lands in the retune's dropped-lock window, sees _stopInProgress != null, and parks
        // on it. EnteredStopInProgressWaitForTesting fires only from that parked wait-out (the retune itself is
        // the first stopper and never hits the _stopInProgress loop), so it uniquely marks "the command start
        // is parked."
        var commandStartParked = new ManualResetEventSlim(false);
        service.EnteredStopInProgressWaitForTesting = () => commandStartParked.Set();

        // A command sample_interval (5000ms) distinct from both the original (10000) and the retune's target
        // (20000), so the final native-sampling interval unambiguously identifies which path won. sample_interval
        // (not cpu_report_interval) is what drives native sampling and is recorded in startIntervals; the two are
        // independent cadences now (see Cluster 3), so this race must be pinned on the sampling one.
        var commandStart = new Thread(() => service.StartFromCommand(new[] { ContinuousProfilingCommandTypes.Cpu }, sampleIntervalMs: 5000, cpuReportIntervalMs: null)) { IsBackground = true };
        commandStart.Start();

        // Wait until the command start has genuinely parked on the retune's in-progress stop before releasing
        // the drain (which lets the retune's stop-then-start complete). Releasing earlier would let the retune
        // finish before the command start ever observed the dropped-lock window -- not the interleaving under
        // test. Deterministic barrier in place of a fixed sleep.
        Assert.That(commandStartParked.Wait(TimeSpan.FromSeconds(5)), Is.True, "the command start must have parked on the retune's in-progress stop");
        releaseReadBatch.Set();

        Assert.That(retune.Join(TimeSpan.FromSeconds(10)) && commandStart.Join(TimeSpan.FromSeconds(10)), Is.True,
            "the retune and the concurrent command start must both complete without deadlocking");

        int lastStartInterval;
        lock (startIntervals)
            lastStartInterval = startIntervals[startIntervals.Count - 1];

        Assert.That(lastStartInterval, Is.EqualTo(5000),
            "the command's requested interval must be the one the session ends up running at, not the config retune's 20000 -- the command's start raced the retune's lock-drop and must not be silently dropped");
        Assert.That(nativeStopCount, Is.EqualTo(2),
            "the retune's stop plus the command start's own retune-to-its-interval stop -- the command did not merely no-op on the retune's restart");
        Assert.That(service.IsActive, Is.True, "the command-started session must be active");
        Mock.Assert(() => _nrLogger.Warn(Arg.Matches<string>(m => m.Contains("Timed out")), Arg.IsAny<object[]>()), Occurs.Never(),
            "neither bounded wait should have timed out in this interleaving");

        // This service never routes through the fixture's Dispose (it is a CreateConnectedService instance) --
        // dispose it explicitly so its EventBus subscription does not leak into later tests.
        service.Dispose();
    }
