...
public class OtlpProfileBuilderTests
{
    private static ManagedThreadSample Sample(string name, long span, params string[] frames) =>
        new ManagedThreadSample(name, 1, 0, 0, span, frames);

    private static ManagedThreadSample AgentWorkSample(string name, long span, params string[] frames) =>
        new ManagedThreadSample(name, 1, 0, 0, span, frames, isAgentWork: true);
    ...
    public void Build_drops_frame_matched_sample_that_is_not_isAgentWork_flagged_when_includeAgentCode_false()
    {
        ...
        Assert.That(req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles, Is.Empty,
            "The only sample was dropped -- no surviving samples, so no profile to emit.");
    }
    ...
    {
        // No sample survived, so no profile is emitted. An empty Profile message is rejected by the
        // OTLP profiles ingest as "no_samples", so we must not emit one.
    ...
    [Test]
    public void Build_emits_exactly_one_cpu_profile()
    {
        // cpu and off_cpu were merged: every surviving sample lands in ONE "cpu" Profile.
        var samples = new[]
        {
            Sample("t1", 0, "A.b"),
            Sample("t2", 0, "C.d"),
    ...
    public void Build_sets_profile_metadata_sample_type_time_and_duration()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(dict.StringTable[profile.SampleType.TypeStrindex], Is.EqualTo("cpu"));
    ...
    {
        // periodNanos = 10 s sampling interval in nanoseconds. The merged profile's period_type matches its
        // sample_type: ("cpu","nanoseconds"), per profiles.proto's own sample_type/period_type pairing.
    ...
    [Test]
    public void Build_EmitsThreadIdAttribute_AsManagedThreadId_WhenRegistryHasAMapping()
    {
        // OS TID chosen far outside any plausible ManagedThreadId range, so this test cannot pass against
        // old code that echoed the OS TID verbatim -- it must actually exercise the registry lookup.
        const long osThreadId = 40_000L;
        var fakeOsThreadIdProvider = Mock.Create<ICurrentOsThreadIdProvider>();
        Mock.Arrange(() => fakeOsThreadIdProvider.GetCurrentOsThreadId()).Returns(osThreadId);
        var realRegistry = new ManagedThreadIdRegistry(fakeOsThreadIdProvider);
        realRegistry.EnsureRegistered(); // records (osThreadId -> this test thread's real ManagedThreadId)
        var expectedManagedThreadId = System.Threading.Thread.CurrentThread.ManagedThreadId;
        var previousInstance = ManagedThreadIdRegistry.Instance;
        ManagedThreadIdRegistry.Instance = realRegistry;

        try
        {
            var req = OtlpProfileBuilder.Build(
                new[] { new ManagedThreadSample("worker-7", osThreadId, 0, 0, 0, new[] { "A()" }) },
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(attrs["thread.id"].IntValue, Is.EqualTo(expectedManagedThreadId));
    ...
    [Test]
    public void Build_OmitsThreadIdAttribute_WhenRegistryHasNoMapping()
    {
        // A fresh, empty real registry -- OS TID 42 was never registered against it, so the lookup misses.
        var emptyRegistry = new ManagedThreadIdRegistry(Mock.Create<ICurrentOsThreadIdProvider>());
        var previousInstance = ManagedThreadIdRegistry.Instance;
        ManagedThreadIdRegistry.Instance = emptyRegistry;

        try
        {
            var req = OtlpProfileBuilder.Build(
                new[] { new ManagedThreadSample("worker-7", 42, 0, 0, 0, new[] { "A()" }) },
                1000, 1, "svc", periodNanos: 1_000_000L);
            var dict = req.Dictionary;
            var sample = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0].Samples.Single();
            ...
            Assert.Multiple(() =>
            {
                Assert.That(attrs.ContainsKey("thread.id"), Is.False);
                Assert.That(attrs.ContainsKey("thread.name")); // thread.name must still be emitted independently
                Assert.That(attrs["thread.name"].StringValue, Is.EqualTo("worker-7"));
            });
        }
        finally
    ...
    {
        var req = OtlpProfileBuilder.Build(
            new[] { new ManagedThreadSample("t1", 1, 0x1122334455667788L, 0x0102030405060708L, unchecked((long)0xAABBCCDDEEFF0011UL), new[] { "A()" }) },
            1000, 1, "svc", periodNanos: 1_000_000L);
        var dict = req.Dictionary;
        var sample = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0].Samples.Single();
    ...
    public void Build_interns_reused_link_across_samples()
    {
        var req = OtlpProfileBuilder.Build(
            new[]
            {
                new ManagedThreadSample("t1", 1, 1, 2, 3, new[] { "A()" }),
                new ManagedThreadSample("t2", 1, 1, 2, 3, new[] { "B()" })
            },
            1000, 1, "svc");
    ...
    [Test]
    public void Build_withPeriod_emitsOneCpuProfileCarryingEverySample()
    {
        var samples = new[]
        {
            Sample("t1", 0, "A.b"),
        ...
        Assert.That(p, Has.Count.EqualTo(1));
        Assert.That(TypeName(p[0]), Is.EqualTo("cpu"));
        Assert.That(p[0].Samples, Has.Count.EqualTo(2));
        Assert.That(p[0].Samples[0].Values[0], Is.EqualTo(1_000_000L));
        Assert.That(p[0].Samples[1].Values[0], Is.EqualTo(1_000_000L));
    }

    [Test]
    public void Build_allIncludedSamplesLandInTheSingleCpuProfile()
    {
        // Distinct stacks/threads so no two samples aggregate together by identity.
        var samples = new[]
        {
            Sample("t1", 0, "A.b"),
            Sample("t2", 0, "C.d"),
        ...
        Assert.That(p, Has.Count.EqualTo(1));
        Assert.That(p[0].Samples, Has.Count.EqualTo(samples.Length));
    }

    [Test]
    public void Build_withoutPeriod_emitsNoProfiles()
    {
        var samples = new[] { Sample("t1", 0, "A.b") };
    ...
    {
        // Same frame across two samples -> exactly one interned stack.
        var samples = new[]
        {
            new ManagedThreadSample("t1", 1, 0, 0, 0, new[] { "A.b" }),
            new ManagedThreadSample("t2", 2, 0, 0, 0, new[] { "A.b" }),
        };
        var req = OtlpProfileBuilder.Build(samples, 0, 0, "svc", periodNanos: 1_000_000L, includeAgentCode: true);
    ...
    [Test]
    public void Build_excludeAgentCode_appliesToTheMergedProfile()
    {
        var samples = new[]
        {
            new ManagedThreadSample("agent", 1, 0, 0, 0, new[] { "NewRelic.Agent.Core.Foo.Bar" }),
            new ManagedThreadSample("app-1", 2, 0, 0, 0, new[] { "App.Work" }),
            new ManagedThreadSample("app-2", 3, 0, 0, 0, new[] { "App.Idle" }),
        };
        ...
        Assert.That(profiles[0].Samples, Has.Count.EqualTo(2)); // the agent sample is dropped
    }

    [Test]
    public void Build_drops_empty_frame_sample_instead_of_colliding_with_the_no_stack_sentinel()
    {
        // A zero-frame sample's stack was previously identical in value to the reserved stack_table[0]
        // "no stack" sentinel, so it silently resolved to stack_index 0 -- indistinguishable on the wire
        // from a sample with no stack at all, which ingest likely dropped silently. The fix drops the
        // empty-frame sample before interning rather than emitting an ambiguous stack_index 0.
        var req = OtlpProfileBuilder.Build(
            new[] { new ManagedThreadSample("t1", 1, 0, 0, 0, System.Array.Empty<string>()) },
            1000, 1, "svc", periodNanos: 1_000_000L);

        Assert.That(req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles, Is.Empty,
            "the only sample had no frames and must be dropped -- no surviving samples, so no profile is emitted");
    }

    [Test]
    public void Build_drops_only_the_empty_frame_sample_and_keeps_siblings_with_real_stacks()
    {
        var samples = new[]
        {
            new ManagedThreadSample("empty", 1, 0, 0, 0, System.Array.Empty<string>()),
            new ManagedThreadSample("real", 2, 0, 0, 0, new[] { "A()" }),
        };
        ...
        Assert.Multiple(() =>
        {
            Assert.That(profile.Samples, Has.Count.EqualTo(1), "only the real-stack sample should survive");
            Assert.That(profile.Samples.Single().StackIndex, Is.Not.EqualTo(0), "the surviving sample's stack must not be the zero sentinel");
            Assert.That(dict.StackTable, Has.Count.EqualTo(2), "zero-value entry + one real interned stack, none for the dropped sample");
        });
    }
    ...
    {
        // Two identical samples for the same thread/stack (e.g. parked across two sweeps in the same
        ...
        var samples = new[]
        {
            new ManagedThreadSample("worker", 7, 0, 0, 0, new[] { "A()" }),
            new ManagedThreadSample("worker", 7, 0, 0, 0, new[] { "A()" }),
        };
        var req = OtlpProfileBuilder.Build(samples, 1000, 1, "svc", periodNanos: 1_000_000L);
        var profile = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0];
    ...
    public void Build_does_not_merge_samples_with_different_stack_identity()
    {
        ...
        {
            new ManagedThreadSample("worker", 7, 0, 0, 0, new[] { "A()" }),
            new ManagedThreadSample("worker", 7, 0, 0, 0, new[] { "B()" }),
        };
        var req = OtlpProfileBuilder.Build(samples, 1000, 1, "svc", periodNanos: 1_000_000L);
        var profile = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0];
    ...
    // --- Cluster H fixes 2 & 3: per-sample time credit (ComputeSampleValueNanos + perSampleValueNanos) ---

    [Test]
    public void ComputeSampleValueNanos_with_no_actual_period_and_no_deferred_uses_the_configured_period()
    {
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, 0, 0, 0), Is.EqualTo(1_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_prefers_the_actual_tick_period_over_the_configured_interval()
    {
        // fix 2: native measured a 20ms real period (capture overran the 10ms configured interval) -> credit
        // 20ms per sample, not the configured period.
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(10_000_000L, actualSamplePeriodMs: 20, emittedThreadCount: 0, deferredThreadCount: 0),
            Is.EqualTo(20_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_scales_by_the_round_robin_factor_when_threads_were_deferred()
    {
        // fix 3: 100 emitted + 300 deferred == 400 live threads, capacity ~100 -> ceil(400/100) == 4, so each
        // emitted sample stands in for 4 periods of its thread's time. The 400-thread w3wp example from the
        // review: totals were ~4x low; this restores the factor.
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, 0, emittedThreadCount: 100, deferredThreadCount: 300),
            Is.EqualTo(4_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_rounds_the_factor_up_for_a_non_multiple()
    {
        // 100 emitted + 350 deferred == 450 -> ceil(450/100) == 5.
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, 0, 100, 350), Is.EqualTo(5_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_composes_the_actual_period_and_the_round_robin_factor()
    {
        // Both corrections at once: a 2ms real period AND a factor-4 deferral -> 2ms * 4.
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, actualSamplePeriodMs: 2, emittedThreadCount: 100, deferredThreadCount: 300),
            Is.EqualTo(8_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_does_not_scale_when_nothing_was_deferred()
    {
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, 0, emittedThreadCount: 100, deferredThreadCount: 0),
            Is.EqualTo(1_000_000L));
    }

    [Test]
    public void ComputeSampleValueNanos_does_not_divide_by_zero_when_emitted_is_zero()
    {
        // Defensive: a stats record with 0 emitted but non-zero deferred (all visited threads failed their
        // snapshot) must not divide by zero -- leave the base value unscaled.
        Assert.That(OtlpProfileBuilder.ComputeSampleValueNanos(1_000_000L, 0, emittedThreadCount: 0, deferredThreadCount: 300),
            Is.EqualTo(1_000_000L));
    }

    [Test]
    public void Build_uses_perSampleValueNanos_for_sample_values_but_keeps_the_configured_period()
    {
        // The per-sample VALUE takes the corrected credit; the informational profile.Period stays the
        // configured interval so the reported cadence isn't distorted by the correction.
        var req = OtlpProfileBuilder.Build(new[] { Sample("t1", 0, "A()") }, 1000, 1, "svc",
            periodNanos: 1_000_000L, perSampleValueNanos: 4_000_000L);
        var profile = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0];

        Assert.Multiple(() =>
        {
            Assert.That(profile.Samples.Single().Values.Single(), Is.EqualTo(4_000_000L), "value must use the override");
            Assert.That(profile.Period, Is.EqualTo(1_000_000L), "informational period stays the configured interval");
        });
    }

    [Test]
    public void Build_falls_back_to_the_period_when_perSampleValueNanos_is_zero()
    {
        var req = OtlpProfileBuilder.Build(new[] { Sample("t1", 0, "A()") }, 1000, 1, "svc",
            periodNanos: 1_000_000L, perSampleValueNanos: 0);
        var profile = req.ResourceProfiles.Single().ScopeProfiles.Single().Profiles[0];

        Assert.That(profile.Samples.Single().Values.Single(), Is.EqualTo(1_000_000L));
    }
}
