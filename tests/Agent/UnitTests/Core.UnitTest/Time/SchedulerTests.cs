...
public class SchedulerTests
{
    ...
    // Regression: SetAgentWork/ResetAgentWork are the two halves of a native per-thread nesting-DEPTH
    // counter that must be paired 1:1. The callback previously called SetAgentWork BEFORE the try whose
    // finally runs ResetAgentWork, so a throwing IContinuousProfilingContext.SetAgentWork would orphan the
    // increment (reset never runs) and escape unhandled onto the ThreadPool thread. SetAgentWork now lives
    // inside the try, so the reset is structurally guaranteed regardless of whether the set throws.
    [Test]
    public void ExecuteOnce_ResetsAgentWork_EvenWhenSetAgentWorkThrows()
    {
        var mockContext = Mock.Create<IContinuousProfilingContext>();
        var resetCalled = new ManualResetEventSlim(false);
        Mock.Arrange(() => mockContext.SetAgentWork()).Throws(new Exception());
        Mock.Arrange(() => mockContext.ResetAgentWork()).DoInstead(() => resetCalled.Set());
        ContinuousProfilingContext.Instance = mockContext;

        using (new TestUtilities.Logging())
        {
            _scheduler.ExecuteOnce(() => { }, TimeSpan.FromMilliseconds(1));

            Assert.That(resetCalled.Wait(TimeSpan.FromSeconds(5)), Is.True,
                "ResetAgentWork must still run when SetAgentWork throws");
        }
    }

    [Test]
    public void ExecuteEvery_ResetsAgentWork_EvenWhenSetAgentWorkThrows()
    {
        var mockContext = Mock.Create<IContinuousProfilingContext>();
        var resetCalled = new ManualResetEventSlim(false);
        Mock.Arrange(() => mockContext.SetAgentWork()).Throws(new Exception());
