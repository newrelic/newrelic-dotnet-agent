...
public class SegmentTests
{
    ...
    [Test]
    public void TryGetMaterializedSpanId_returns_null_before_the_span_id_is_generated()
    {
        var segment = new Segment(TransactionSegmentStateHelpers.GetItransactionSegmentState(), new MethodCallData("Type", "Method", 1));

        Assert.That(segment.TryGetMaterializedSpanId(), Is.Null);
    }

    // The load-bearing property: probing must not itself mint an id, or continuous profiling would
    // manufacture span ids for spans that are never reported.
    [Test]
    public void TryGetMaterializedSpanId_does_not_generate_a_span_id()
    {
        var segment = new Segment(TransactionSegmentStateHelpers.GetItransactionSegmentState(), new MethodCallData("Type", "Method", 1));

        segment.TryGetMaterializedSpanId();

        Assert.That(segment.TryGetMaterializedSpanId(), Is.Null);
    }

    [Test]
    public void TryGetMaterializedSpanId_returns_the_id_once_it_has_been_generated()
    {
        var segment = new Segment(TransactionSegmentStateHelpers.GetItransactionSegmentState(), new MethodCallData("Type", "Method", 1));
        var generated = segment.SpanId;

        Assert.That(segment.TryGetMaterializedSpanId(), Is.EqualTo(generated));
        Assert.That(segment.TryGetMaterializedSpanId(), Is.SameAs(generated));
    }

    [Test]
    public void TryGetMaterializedSpanId_returns_an_explicitly_assigned_id()
    {
        var segment = new Segment(TransactionSegmentStateHelpers.GetItransactionSegmentState(), new MethodCallData("Type", "Method", 1));
        segment.SpanId = "1122334455667788";

        Assert.That(segment.TryGetMaterializedSpanId(), Is.EqualTo("1122334455667788"));
    }
