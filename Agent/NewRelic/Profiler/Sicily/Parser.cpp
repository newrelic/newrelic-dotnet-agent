...
namespace sicily {
    ...
    Parser::ParseMethodSignature(Scanner &scanner, bool instanceMethod, bool methodRequired)
    {
        ...
        {
            throw UnexpectedTypeKindException(tempTargetType->GetKind(), ast::Type::kCLASS);
        }
    ...
    Parser::ParseTypeSignature(Scanner &scanner, bool allowRawClassName)
    {
        ...
        switch (type) {
            case TOK_OBJECT:
                result = std::make_shared<ast::PrimitiveType>(ast::PrimitiveType::kOBJECT);
                break;
            case TOK_VOID:
                result = std::make_shared<ast::PrimitiveType>(ast::PrimitiveType::kVOID);
                break;
            case TOK_BOOL:
                result = std::make_shared<ast::PrimitiveType>(ast::PrimitiveType::kBOOL);
                break;
			case TOK_UINT32:
				result = std::make_shared<ast::PrimitiveType>(ast::PrimitiveType::kU4);
				break;
			case TOK_STRING:
                result = std::make_shared<ast::PrimitiveType>(ast::PrimitiveType::kSTRING);
                break;
            case TOK_CLASS:
