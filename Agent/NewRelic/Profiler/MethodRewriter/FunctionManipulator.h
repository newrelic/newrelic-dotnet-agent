...
//#define TEST_UNSAFE 1
	...
	public:
		FunctionManipulator(IFunctionPtr function) : FunctionManipulator(function, function->GetMethodBytes(false))
		{ }

		FunctionManipulator(IFunctionPtr function, ByteVectorPtr methodBytes) :
			_function(function),
		...
		{
			_valid = _methodBytes != nullptr;

			if (_valid)
			{
				ExtractHeaderBodyAndExtra(_methodBytes);
		...
		// Writes the function if everything is okay.  Returns true if the function was written.
		bool WriteFunction()
		{
			if (_valid)
			{
				return DoWriteFunction();
			}
			return _valid;
		}

	protected:
		virtual bool DoWriteFunction() = 0;
		...
		void ExtractHeaderBodyAndExtra(ByteVectorPtr& originalMethodBytes)
		{
			...
			// TODO: investigate whether or not we need CorILMethod_InitLocals, I don't think we do and it could help performance
