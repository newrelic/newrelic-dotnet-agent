...
#pragma warning(push)
#pragma warning(disable : 4100 4245 4456 4189)
	...
	PVOID CallbackContext)
{
	...
	LOG_TRACE(L"ProfilerControlCallback DoETWCommand IsEnabled 0x%lx Level 0x%x MatchAny 0x%llx\n", IsEnabled, Level, MatchAnyKeywords);
	...
	/* [in] */ UINT cbClientData)
{
	...
	if (m_profilerLoadedAtStartup)
	{
		...
		CALL_N_LOGONBADHR(m_info->SetEventMask(oldFlags | (COR_PRF_ENABLE_OBJECT_ALLOCATED | COR_PRF_MONITOR_GC)));
...
void CorProfilerTracer::ForceGC()
{
	...
	LOG_TRACE(L"ForceGC: thread 0x%p\n", thread);
...
ClassInfo* CorProfilerTracer::GetClassInfo(ClassID classId)
{
	...
	if (classInfo->ID == 0)
	{
		...
		{
			LOG_TRACE(L"Error getting information for class ID 0x%llx\n", (uint64_t)classId);
		}
	}
