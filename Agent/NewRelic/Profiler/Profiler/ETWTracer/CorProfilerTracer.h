...
#define LOG_TRACE(...)                            \
	{                                             \
		wchar_t buff[3000];                       \
		*buff = L'\0';                            \
		auto len = ::swprintf_s(buff, 3000, __VA_ARGS__); \
		if (L'\n' == buff[len - 1])               \
			buff[len - 1] = L'\0';                \
		LogTrace(buff);                           \
	}
...
#define ETWTRACER_IMPL(etwTracer) \
virtual STDMETHODIMP MovedReferences(ULONG cMovedObjectIDRanges, ObjectID oldObjectIDRangeStart[], ObjectID newObjectIDRangeStart[], ULONG cObjectIDRangeLength[]) override \
{ \
	return etwTracer.MovedReferences(cMovedObjectIDRanges, oldObjectIDRangeStart, newObjectIDRangeStart, cObjectIDRangeLength); \
} \
virtual STDMETHODIMP ObjectAllocated(ObjectID objectId, ClassID classId) override \
{ \
	return etwTracer.ObjectAllocated(objectId, classId); \
} \
 \
virtual STDMETHODIMP ObjectReferences(ObjectID objectId, ClassID classId, ULONG cObjectRefs, ObjectID objectRefIds[]) override \
{ \
	return etwTracer.ObjectReferences(objectId, classId, cObjectRefs, objectRefIds); \
} \
 \
virtual STDMETHODIMP GarbageCollectionStarted(int cGenerations, BOOL generationCollected[], COR_PRF_GC_REASON reason) override \
{ \
	return etwTracer.GarbageCollectionStarted(cGenerations, generationCollected, reason); \
} \
 \
virtual STDMETHODIMP SurvivingReferences(ULONG cSurvivingObjectIDRanges, ObjectID objectIDRangeStart[], ULONG cObjectIDRangeLength[]) override \
{ \
	return etwTracer.SurvivingReferences(cSurvivingObjectIDRanges, objectIDRangeStart, cObjectIDRangeLength); \
} \
 \
virtual STDMETHODIMP GarbageCollectionFinished(void) override \
{ \
	return etwTracer.GarbageCollectionFinished(); \
} \
 \
virtual STDMETHODIMP FinalizeableObjectQueued(DWORD finalizerFlags, ObjectID objectID) override \
{ \
	return etwTracer.FinalizeableObjectQueued(finalizerFlags, objectID); \
} \
 \
virtual STDMETHODIMP RootReferences2(ULONG cRootRefs, ObjectID rootRefIds[], COR_PRF_GC_ROOT_KIND rootKinds[], COR_PRF_GC_ROOT_FLAGS rootFlags[], UINT_PTR rootIds[]) override \
{ \
	return etwTracer.RootReferences2(cRootRefs, rootRefIds, rootKinds, rootFlags, rootIds); \
} \
 \
virtual STDMETHODIMP HandleCreated(GCHandleID handleId, ObjectID initialObjectId) override \
{ \
	return etwTracer.HandleCreated(handleId, initialObjectId); \
} \
 \
virtual STDMETHODIMP HandleDestroyed(GCHandleID handleId) override \
{ \
	return etwTracer.HandleDestroyed(handleId); \
} 
