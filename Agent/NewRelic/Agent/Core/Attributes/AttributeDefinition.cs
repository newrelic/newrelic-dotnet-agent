...
namespace NewRelic.Agent.Core.Attributes
{
	...
	{
		private static readonly AttributeDestinations[] AttribDestinationValues = Enum.GetValues(typeof(AttributeDestinations)).OfType<AttributeDestinations>().ToArray();
		...
		public AttributeDefinition<TInput, TOutput> Build(IAttributeFilter filter)
		{
			...
			if (_classification != AttributeClassification.Intrinsics)
			{
				...
				{
					_availability[dest] = _availability[dest] && !filter.ShouldExcludeAttribute(_name, dest, dest);
				}
			}
	...
	public class AttributeDefinition
	{
		...
		public AttributeDefinition(string name, AttributeClassification classification, Dictionary<AttributeDestinations, bool> availability)
		{
			...
			foreach (var k in availability.Keys)
			{
				AttributeDestinations |= k;
			}
		}

		public bool IsAvailableFor(AttributeDestinations targetModel)
		{
			return _availability.TryGetValue(targetModel, out var isAvail) && isAvail;
		}
	...
	public class AttributeDefinition<TInput, TOutput> : AttributeDefinition
	{
		...
		{
			if (!IsDefinitionValid || collection.IsImmutable || !IsAvailableFor(collection.TargetModelType))
			{
				return false;
			}
			...
			return collection.TrySetValue(this, new Lazy<object>(ResolveLazyValue));

			object ResolveLazyValue()
			{
				return getInputValFx();
			}
		}
		...
		public bool TrySetDefault(IAttributeValueCollection collection)
		{
			...
			if (!IsAvailableFor(collection.TargetModelType))
			{
				return false;
			}
