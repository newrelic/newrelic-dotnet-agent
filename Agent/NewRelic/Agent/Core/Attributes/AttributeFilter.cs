	...
	public class AttributeFilter : IAttributeFilter
	{
		...
		public IEnumerable<Attribute> FilterAttributes(IEnumerable<Attribute> attributes, AttributeDestinations destination, bool logInvalidAttribs)
		{
			...
			foreach (var attr in attributes)
			{
				if(!attr.IsValid)
				{
					if (logInvalidAttribs)
					{
						LogInvalidAttribute(destination, attr);
					}
					continue;
				}
		...
		/// <summary>
		/// Attribute spec indicates that we should log invalid attributes.
		/// </summary>
		/// <param name="destination"></param>
		/// <param name="attribute"></param>
		private void LogInvalidAttribute(AttributeDestinations destination, Attribute attribute)
		{
			if (attribute.IsInvalidKeyEmpty) Log.Debug($"{destination} Attribute could not be added - Key is null/empty (value='{attribute.Value})'");
			if (attribute.IsInvalidKeyTooLarge) Log.Debug($"{destination} Attribute could not be added - Key is too large (key='{attribute.Key}')");
